...
struct MiaFrame
{
    MiaFrame() : stream_index(0), frame_number(0), callback(NULL)
    {
        CLEAR(mibuffer);
...
enum ResultType { MIA_SERVICE_DIED, MIA_RESULT_METADATA, MIA_ANCHOR_CALLBACK = 10};
...
{
    SessionOutput() : type(SESSION_OUT_BUFFERS) { CLEAR(surfaces); }
...
enum MsgType {
    ...
    MIA_MSG_SERVICE_DIED,    // service notifications, service died
    MIA_MSG_ABNORMAL_PROCESSING  //mivi abnormal processing
};
