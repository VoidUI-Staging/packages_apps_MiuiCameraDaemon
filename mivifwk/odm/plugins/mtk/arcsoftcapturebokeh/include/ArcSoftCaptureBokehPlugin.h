...
private:
    ~ArcCapBokehPlugin();
#if defined(ZEUS_CAM)
    MiDualCamBokeh *mMiDualCamBokeh;
#endif
    ...
    int processBuffer(std::vector<ImageParams> teleInputvector,
                      std::vector<ImageParams> wideInputvector, struct MiImageBuffer *bokehOutput,
                      struct MiImageBuffer *teleOutput, struct MiImageBuffer *depthOut);
    int iniEngine();
    ...
    void mapRectToInputData(MIRECT &srcRect, MIRECT &IFERect, int &nDataWidth, int &nDataHeight,
                            MIRECT &dstRectOnInputData);
    void convertToArcFaceInfo(FDMetadataResults &fdMeta, ARC_DCR_FACE_PARAM &faceinfo,
                              MIRECT &IFERect, int &nDataWidth, int &nDataHeight);
