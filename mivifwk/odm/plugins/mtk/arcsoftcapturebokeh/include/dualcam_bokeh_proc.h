...
enum ALGO_BEAUTY_LENS_MODE { BMODE_PORTRAIT, BMODE_FOOD, BMODE_PET, BMODE_OBJECT, BMODE_OFF };
...
enum ALGO_CAPTURE_BOKEH_ZOOM_RATIO { CAPTURE_BOKEH_2X = 0, CAPTURE_BOKEH_1X = 1 };
...
enum ALGO_CAPTURE_BOKEH_MODE { CAPTURE_BOKEH_QUALITY_MODE = 0, CAPTURE_BOKEH_PERFORMANCE_MODE = 1 };

enum ALGO_CAPTURE_BOKEH_DEBUG { CAPTURE_BOKEH_DEBUG_OFF = 0, CAPTURE_BOKEH_DEBUG_ON = 1 };
...
typedef struct AlgoBokehInterface
{
    ...
    int motionFlag =
        0;        // [in]  The trigger motion flag for serial scenes, new scene as 0, same as 1.
...
typedef struct AlgoBokehLaunchParams
{
    ...
    ALGO_CAPTURE_BOKEH_MODE
    bokehMode; // [in]  The Switch between bokeh performance and normal version
    ...
    unsigned int thermallevel = (unsigned int)0; // [in] The Phone temperature 0: Low < 35; 1: Enter
                                                 // 35 ~ 37;2: mid  46 ~48; 3: High 49 ~ 50
 ...
 ************************************************************************/
int MIALGO_CaptureInitWhenLaunch( // return 0 if success, otherwise fail, only called once when
                                  // launching
...
int MIALGO_CaptureDestoryWhenClose( // return 0 if success, otherwise fail, only called once when
                                    // node deInit.
    void **phEngine                 // [in]  The algorithm engine.
);
