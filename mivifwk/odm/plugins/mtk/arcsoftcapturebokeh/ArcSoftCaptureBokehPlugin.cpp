...
#define MI_AIIEALGO_SNPE_PATH   "/vendor/etc/camera/model_aiie_cap.dlc"
...
static const int32_t sArchsoftCbLogLevel =
    property_get_int32("persist.vendor.camera.arcsoft.cb.loglevel", 1);
...
static const int32_t sDebugAbort = property_get_int32("persist.vendor.camera.bokeh.sigabrt", 0);
...
static void *bokeh_AIIE_proc(MHandle pHandle, LPASVLOFFSCREEN pInputImage)
{
#if defined(FEATURE_ASD_ENABLE)
    int result = 0;
    ...
    stInitInfo.image_width = pInputImage->i32Width;
    stInitInfo.image_height = pInputImage->i32Height;
    stInitInfo.pHandle = pHandle;

    result = ALGO_IE_CaptureInit(stInitInfo, stInitOut);

    if (result != 0 || NULL == pHandle) {
        MLOGE(Mia2LogGroupPlugin, " [ARC_CTB_AIIE] AIIE Init Params Faild res = %d", result);
        return NULL;
    }
    ...
    stInputInfo.iImgDataYFD = 1; // fd not used yet
    stInputInfo.iImgDataUVFD = 1;
    stInputInfo.pHandle = pHandle;
    stInputInfo.iImgHeight = pInputImage->i32Height;
    stInputInfo.iImgWidth = pInputImage->i32Width;
    stInputInfo.iImgStride = pInputImage->pi32Pitch[0];
    stInputInfo.pImgDataY = pInputImage->ppu8Plane[0];
    stInputInfo.pImgDataUV = pInputImage->ppu8Plane[1];

    if (pInputImage->u32PixelArrayFormat == ASVL_PAF_NV12) {
        stInputInfo.iImgFormat = 2;
    } else if (pInputImage->u32PixelArrayFormat == ASVL_PAF_NV21) {
        stInputInfo.iImgFormat = 1;
    }

    stOutput.iOutDataYFD = 1;
    stOutput.iOutDataUVFD = 1;
    stOutput.iImgWidth = pInputImage->i32Width;
    stOutput.iImgHeight = pInputImage->i32Height;
    stOutput.iImgStride = pInputImage->pi32Pitch[0];
    stOutput.pOutDataY = pInputImage->ppu8Plane[0];
    stOutput.pOutDataUV = pInputImage->ppu8Plane[1];

    result = ALGO_IE_CaptureProcess(stInputInfo, stOutput);

    if (result != 0) {
        MLOGE(Mia2LogGroupPlugin, " [ARC_CTB_AIIE] AIIE Process Faild res = %d", result);
        return NULL;
    }
#endif
    return NULL;
...
static void *bokeh_relighting_thread_proc_prepare(void *params)
{
    int nRet = MOK;
    double t0, t1;
    relight_thread_params *rtp = (relight_thread_params *)params;

    // relight init
    MLOGD(Mia2LogGroupPlugin, " [ARC_CTB_RLT] init start");

    int width = rtp->pSrcImg.i32Width;
    int height = rtp->pSrcImg.i32Height;
    MHandle mRelightingHandle = NULL;

    t0 = gettime();
    nRet = AILAB_PLI_Init(&mRelightingHandle, MI_PL_INIT_MODE_PERFORMANCE, width, height);
    t1 = gettime();
    if (nRet != MOK) {
        mRelightingHandle = NULL;
        MLOGE(Mia2LogGroupPlugin, " [ARC_CTB_RLT] AILAB_PLI_Init failed: %d", nRet);
    } else {
        MLOGD(Mia2LogGroupPlugin,
              " [ARC_CTB_RLT] AILAB_PLI_Init = %d: cost=%.2f, mRelightingHandle:%p\n", nRet,
              t1 - t0, mRelightingHandle);
    }

    // relight preprocess
    if (nRet == MOK) {
        MLOGD(Mia2LogGroupPlugin, " [ARC_CTB_RLT] preprocess start");

        MIPLLIGHTREGION mLightRegion = {0};

        MLOGD(Mia2LogGroupPlugin,
              " [ARC_CTB_RLT] AILAB_PLI_PreProcess src i32Width %d i32Height %d ",
              rtp->pSrcImg.i32Width, rtp->pSrcImg.i32Height);
        MLOGD(Mia2LogGroupPlugin,
              " [ARC_CTB_RLT] AILAB_PLI_PreProcess dst i32Width %d i32Height %d ",
              rtp->pDstImg.i32Width, rtp->pDstImg.i32Height);

        t0 = gettime();
        nRet = AILAB_PLI_PreProcess(mRelightingHandle, &rtp->pSrcImg, rtp->orient, &mLightRegion);
        t1 = gettime();
        if (nRet != MOK) {
            AILAB_PLI_Uninit(&mRelightingHandle);
            mRelightingHandle = NULL;
            MLOGE(Mia2LogGroupPlugin, " [ARC_CTB_RLT] AILAB_PLI_PreProcess failed = %d", nRet);
        } else {
            MLOGD(Mia2LogGroupPlugin, " [ARC_CTB_RLT] AILAB_PLI_PreProcess= %d: preprocess = %.2f",
                  nRet, t1 - t0);
        }
    }

    if (NULL != rtp->m_phRelightingHandle)
        *(rtp->m_phRelightingHandle) = mRelightingHandle;
    else
        MLOGE(Mia2LogGroupPlugin, " [ARC_CTB_RLT] rtp->m_phRelightingHandle is NULL !");
...
int ArcCapBokehPlugin::deInit()
{
...
#if defined(FEATURE_MI_BOKEH_LITE)
    if (mMiDualCamBokeh) {
        // mMiDualCamBokeh->deInit(); //mialgo Lite is not uesd
        delete mMiDualCamBokeh;
        mMiDualCamBokeh = NULL;
    }
#endif
    ...
    if (mAIIEHandle != NULL) {
        ...
        stFreeInfo.pHandle = mAIIEHandle;
        ALGO_IE_CaptureFree(stFreeInfo);

        MLOGD(Mia2LogGroupPlugin, " [ARC_CTB_AIIE] AIIE DeInit model Successfull handle is %p",
              mAIIEHandle);
        mAIIEHandle = NULL;
    }
...
int ArcCapBokehPlugin::initialize(CreateInfo *createInfo, MiaNodeInterface nodeInterface)
{
...
#if defined(FEATURE_MI_BOKEH_LITE)
    mMiDualCamBokeh = NULL;
    if (mMiDualCamBokeh == NULL) {
        mMiDualCamBokeh = new MiDualCamBokeh();
        // mMiDualCamBokeh->Init(mFrameworkCameraId); //mialgo Lite is not uesd
    }
#endif
    mAIIEHandle = NULL;
    mIsAIIEEnabled = false;
    ...
    MLOGI(Mia2LogGroupPlugin,
          " [ARC_CTB] init mRelightingHandle to %p, mRearCamera:%d, cameraId: 0x%x, mSDKMode:%d",
          mRelightingHandle, mRearCamera, mFrameworkCameraId, mSDKMode);
...
int ArcCapBokehPlugin::iniEngine()
{
    ...
    // bokeh has no AI function, remove AIIE in bokeh when Android T branch
#if defined(FEATURE_ASD_ENABLE)
    // Start init AIIE model, do this operation only once
    // Note: Init and Destroy appear in pairs
    ALGO_IE_CAPTURE_INITPARAM stInitInfo;
    ALGO_IE_CAPTURE_INITOUT_DL stInitOut;

    memset(&stInitInfo, 0, sizeof(stInitInfo));
    memset(&stInitOut, 0, sizeof(stInitOut));

    stInitInfo.config_path = MI_AIIEALGO_PARAMS_PATH;
    stInitInfo.device_model = 2; // 2 means GPU
    stInitInfo.dlc_path = MI_AIIEALGO_SNPE_PATH;

    int resultAIIEInit = ALGO_IE_ModelInit(stInitInfo, stInitOut);

    if (0 == resultAIIEInit) {
        mAIIEHandle = stInitOut.pHandle;
        MLOGD(Mia2LogGroupPlugin, " [ARC_CTB_AIIE] AIIE Init model Successfull handle is %p",
              mAIIEHandle);
    } else {
        MLOGE(Mia2LogGroupPlugin, " [ARC_CTB_AIIE] AIIE Init model Faild res = %d", resultAIIEInit);
        return PROCFAILED;
    }
    // End init AIIE model
#endif
...
ProcessRetStatus ArcCapBokehPlugin::processRequest(ProcessRequestInfo *processRequestInfo)
{
    ProcessRetStatus resultInfo = PROCSUCCESS;
    int32_t iIsDumpData = (int32_t)sArccapBokehDump;

    int inputFrameCount = processRequestInfo->inputBuffersMap.size();
    mMaxParallelNum = processRequestInfo->maxConcurrentNum;
    mCurrentParallelNum = processRequestInfo->runningTaskNum;
    std::vector<ImageParams> input_tele_vector;
    std::vector<ImageParams> input_wide_vector;

    if (inputFrameCount % 2 != 0) {
        resultInfo = PROCFAILED;
        return resultInfo;
    }
    // inputFrameCount = inputFrameCount / 2;

    for (int i = 0; i < inputFrameCount; i++) {
        auto &inputBuffers = processRequestInfo->inputBuffersMap.at(i);
        if (i % 2 == 0) {
            input_tele_vector.push_back(inputBuffers[0]);
        } else {
            input_wide_vector.push_back(inputBuffers[0]);
        }
    }
    ...
    for (int i = 0; i < inputFrameCount; i++) {
        void *pMetaData = NULL;
        int expValue = -1;
        camera_metadata_t *pMeta = input_tele_vector[i].pMetadata;
        VendorMetadataParser::getTagValue(pMeta, ANDROID_CONTROL_AE_EXPOSURE_COMPENSATION,
                                          &pMetaData);
        if (NULL != pMetaData) {
            expValue = *static_cast<int32_t *>(pMetaData);
        } else {
            MLOGE(Mia2LogGroupPlugin, " [ARC_CTB] get EV failed");
        }
        if (expValue == 0) {
            if (i != 0) {
                ImageParams pTempImageTele, pTempImageWide;
                pTempImageTele = input_tele_vector[0];
                input_tele_vector[0] = input_tele_vector[i];
                input_tele_vector[i] = pTempImageTele;

                pTempImageWide = input_wide_vector[0];
                input_wide_vector[0] = input_wide_vector[i];
                input_wide_vector[i] = pTempImageWide;
            }
            break;
        }
    }
    ...
    mOutputNum = processRequestInfo->outputBuffersMap.size();
    if (mOutputNum == 3) {
        // prot1 ->Tele frame
    ...
    for (int i = 0; i < inputFrameCount; i++) {
        ...
        int teleExpValue = -1;
        int wideExpValue = -1;
        VendorMetadataParser::getTagValue(input_tele_vector[i].pMetadata,
                                          ANDROID_CONTROL_AE_EXPOSURE_COMPENSATION, &pMetaData);
        if (NULL != pMetaData) {
        ...
        pMetaData = NULL;
        VendorMetadataParser::getTagValue(input_wide_vector[i].pMetadata,
                                          ANDROID_CONTROL_AE_EXPOSURE_COMPENSATION, &pMetaData);
        if (NULL != pMetaData) {
        ...
        MLOGI(Mia2LogGroupPlugin,
              ...
              "%d, format: %d",
              i + 1, inputFrameCount, teleExpValue, input_tele_vector[i].format.width,
        ...
        MLOGI(Mia2LogGroupPlugin,
              ...
              "%d, format: %d",
              i + 1, inputFrameCount, wideExpValue, input_wide_vector[i].format.width,
...
#if defined(FEATURE_MI_BOKEH_LITE)
    if (mCalcMode == MI_BOKEH_LITE) {
        bokeh_lite_params liteParams;
        result = mMiDualCamBokeh->ProcessLiteBuffer(teleimagevector, wideimagevector, bokehOutput,
                                                    teleOutput, depthOut, liteParams);
        if (PROCSUCCESS != result) {
            MLOGE(Mia2LogGroupPlugin, " [MI_CTB] ERR! ProcessLiteBuffer failed:%d", result);
        } else {
            mBlurLevel = liteParams.BlurLevel;
            mBeautyEnabled = liteParams.BeautyEnabled;
            mHdrEnabled = liteParams.HdrEnabled;
        }
        return result;
    }
#endif
                                      ...
                                      const std::vector<camera_metadata_t *> &logicalMetaVector)
{
    ...
    const char *hdrenabletag = "xiaomi.hdr.enabled";
    VendorMetadataParser::getVTagValue(logicalMetaVector[0], hdrenabletag, &localMetadata);
    if (NULL == localMetadata) {
        mHdrEnabled = false;
    ...
    camera_metadata_t *pLogicalMetaData = logicalMetaVector[0];
    ...
    const char *supernightenable = "xiaomi.supernight.raw.enabled";
    VendorMetadataParser::getVTagValue(pLogicalMetaData, supernightenable, &localMetadata);
    if (NULL != localMetadata) {
    ...
    VendorMetadataParser::getVTagValue(pLogicalMetaData, thermallevel, &localMetadata);
    if (NULL != localMetadata) {
    ...
    if ((mCurrentParallelNum >= (mMaxParallelNum - 1) || mThermallevel > 1) &&
        mCalcMode == ARC_DCIR_BOKEH_NORMAL) {
        if (isLite == 1 && mCaptureMode == 1) {
            mCalcMode = MI_BOKEH_LITE;
        } else {
            mCalcMode = ARC_DCIR_BOKEH_LITE;
        }
    } else if ((mCalcMode == MI_BOKEH_LITE || mCalcMode == ARC_DCIR_BOKEH_LITE) &&
               mCurrentParallelNum <= 2 && mThermallevel <= 1) {
        mCalcMode = ARC_DCIR_BOKEH_NORMAL;
    }

    if (isLite == 2) { // for debug
        mCalcMode = MI_BOKEH_LITE;
    }

    if (mCalcMode == MI_BOKEH_LITE) {
        mMiDualCamBokeh->m_pLogicalMetaData = pLogicalMetaData;
        return PROCSUCCESS;
    }
#else
    if ((mCurrentParallelNum >= (mMaxParallelNum - 1) || mThermallevel > 1) &&
    ...
    VendorMetadataParser::getVTagValue(pLogicalMetaData, bokehMetadata, &localMetadata);
    if (NULL != localMetadata) {
    ...
    int orient = 0;
    localMetadata = NULL;
    VendorMetadataParser::getTagValue(pLogicalMetaData, ANDROID_JPEG_ORIENTATION, &localMetadata);
    if (NULL != localMetadata) {
    ...
    for (uint8_t i = 0; i < MAX_LINKED_NUMBER; i++) {
        MLOGD(Mia2LogGroupPlugin,
              " [ARC_CTB] cameraMetadata[%d] isValid %d masterCameraId %d cameraId %d "
              "isQuadCFASensor %d qcfaSensorSumFactor %d fovRectIFE [%d %d %d %d] "
              "activeArraySizeTele [%d %d %d %d]",
              i, 0, 0, 0,
              // inputCTB->cameraMetadata[i].isValid, inputCTB->masterCameraId,
              // inputCTB->cameraMetadata[i].cameraId,
              inputCTB->cameraMetadata[i].isQuadCFASensor, 0,
              // inputCTB->cameraMetadata[i].qcfaSensorSumFactor,
              inputCTB->cameraMetadata[i].fovRectIFE.left,
              inputCTB->cameraMetadata[i].fovRectIFE.top,
              inputCTB->cameraMetadata[i].fovRectIFE.width,
              inputCTB->cameraMetadata[i].fovRectIFE.height,
              inputCTB->cameraMetadata[i].activeArraySize.left,
              inputCTB->cameraMetadata[i].activeArraySize.top,
              inputCTB->cameraMetadata[i].activeArraySize.width,
              inputCTB->cameraMetadata[i].activeArraySize.height);

        if (/*inputCTB->cameraMetadata[i].isValid &&
            inputCTB->masterCameraId == inputCTB->cameraMetadata[i].cameraId*/
            inputCTB->cameraMetadata[i].cameraRole == CameraRoleTypeTele) {
            MLOGD(Mia2LogGroupPlugin,
                  " [ARC_CTB]SetFrameParams cameraMetadata[%d].cameraRole is Tele)", i);
            int inputDataWidth = stTeleImage.i32Width;
            int inputDataHeight = stTeleImage.i32Height;
            MIRECT fovRectIFEForTele = inputCTB->cameraMetadata[i].fovRectIFE;
            MIRECT activeArraySizeTele = inputCTB->cameraMetadata[i].activeArraySize;

            if (mRearCamera) {
                uint32_t sensorW = fovRectIFEForTele.width;
                uint32_t sensorH = fovRectIFEForTele.height;
                // int32_t qcfaSensorSumFactor = inputCTB->cameraMetadata[i].qcfaSensorSumFactor;
                if (true == inputCTB->cameraMetadata[i].isQuadCFASensor) {
                    sensorW /= 2;
                    sensorH /= 2;
                }
                mFrameParams.caminfo.i32MainWidth_CropNoScale = sensorW;
                mFrameParams.caminfo.i32MainHeight_CropNoScale =
                    sensorW * inputDataHeight / inputDataWidth;
            } else {
                mFrameParams.caminfo.i32MainWidth_CropNoScale = 2872;
                mFrameParams.caminfo.i32MainHeight_CropNoScale = 2154;
            }

            focusROI = inputCTB->cameraMetadata[i].afFocusROI;
            ...
            memset(&mRtFocusROIOnInputData, 0, sizeof(MIRECT));
            mapRectToInputData(rtFocusROI, fovRectIFEForTele, inputDataWidth, inputDataHeight,
            ...
            if (mRearCamera) {
                ...
                if (mFrameParams.refocus.ptFocus.x == 0 && mFrameParams.refocus.ptFocus.y == 0) {
                    mFrameParams.refocus.ptFocus.x =
                        mFrameParams.mainimg.i32Width / 2; /******hard code*******/
                    mFrameParams.refocus.ptFocus.y =
                        mFrameParams.mainimg.i32Height / 2; /******hard code*******/
                }
            } else {
                MLOGD(Mia2LogGroupPlugin, " [ARC_CTB] capture facenum = %d",
                      inputCTB->cameraMetadata[i].fdMetadata.numFaces);
                if (inputCTB->cameraMetadata[i].fdMetadata.numFaces > 0) {
                    unsigned int mLargeFaceRegion = 0;
                    MIRECT rtOnInputData;
                    memset(&rtOnInputData, 0, sizeof(MIRECT));
                    mapRectToInputData(inputCTB->cameraMetadata[i].fdMetadata.faceRect[0],
                                       fovRectIFEForTele, inputDataWidth, inputDataHeight,
                                       rtOnInputData);
                    mFrameParams.refocus.ptFocus.x =
                    ...
                    MLOGD(Mia2LogGroupPlugin,
                          " [ARC_CTB] capture facerect[0](l,t,w,h)[%d,%d,%d,%d]",
                          rtOnInputData.left, rtOnInputData.top, rtOnInputData.width,
                          rtOnInputData.height);

                    for (int nFaceIndex = 1;
                         nFaceIndex < inputCTB->cameraMetadata[i].fdMetadata.numFaces;
                         ++nFaceIndex) {
                        memset(&rtOnInputData, 0, sizeof(MIRECT));
                        mapRectToInputData(
                            inputCTB->cameraMetadata[i].fdMetadata.faceRect[nFaceIndex],
                            fovRectIFEForTele, inputDataWidth, inputDataHeight, rtOnInputData);
                        ...
                        MLOGD(Mia2LogGroupPlugin,
                              " [ARC_CTB] : capture facerect[%d](l,t,w,h)[%d,%d,%d,%d]", nFaceIndex,
                              rtOnInputData.left, rtOnInputData.top, rtOnInputData.width,
                              rtOnInputData.height);
                    }
                } else {
                    ...
                    mFrameParams.refocus.ptFocus.y =
                        mRtFocusROIOnInputData.top + mRtFocusROIOnInputData.height / 2;
                    if (mFrameParams.refocus.ptFocus.x == 0 ||
                        mFrameParams.refocus.ptFocus.y == 0) {
                        mFrameParams.refocus.ptFocus.x = inputDataWidth / 2;
                        mFrameParams.refocus.ptFocus.y = inputDataHeight / 2;
                    }
                }
            }
            MLOGD(Mia2LogGroupPlugin,
                  " [ARC_CTB] focus rect(l,t,w,h):[%d,%d,%d,%d] ptFocus:[%d,%d]", focusROI.xMin,
                  focusROI.yMin, focusROI.xMax, focusROI.yMax, mFrameParams.refocus.ptFocus.x,
                  mFrameParams.refocus.ptFocus.y);

            MLOGD(Mia2LogGroupPlugin, " [ARC_CTB] tele facenum = %d",
                  inputCTB->cameraMetadata[i].fdMetadata.numFaces);
            for (int nFaceIndex = 0; nFaceIndex < inputCTB->cameraMetadata[i].fdMetadata.numFaces;
                 ++nFaceIndex) {
                MLOGD(Mia2LogGroupPlugin, " [ARC_CTB] facerect[%d](l,t,w,h)[%d,%d,%d,%d]",
                      nFaceIndex, inputCTB->cameraMetadata[i].fdMetadata.faceRect[nFaceIndex].left,
                      inputCTB->cameraMetadata[i].fdMetadata.faceRect[nFaceIndex].top,
                      inputCTB->cameraMetadata[i].fdMetadata.faceRect[nFaceIndex].width,
                      inputCTB->cameraMetadata[i].fdMetadata.faceRect[nFaceIndex].height);
            }

            // for Face info
            convertToArcFaceInfo(inputCTB->cameraMetadata[i].fdMetadata, mBokehParam.faceParam,
                                 fovRectIFEForTele, inputDataWidth, inputDataHeight);

        } else if (/*inputCTB->cameraMetadata[i].isValid*/
                   inputCTB->cameraMetadata[i].cameraRole == CameraRoleTypeWide) {
            MLOGD(Mia2LogGroupPlugin, "[ARC_CTB] cameraMetadata[%d].cameraRole is wide)", i);
            int inputDataWidth = stWideImage.i32Width;
            int inputDataHeight = stWideImage.i32Height;
            MIRECT fovRectIFEForWide = inputCTB->cameraMetadata[i].fovRectIFE;
            MIRECT activeArraySizeWide = inputCTB->cameraMetadata[i].activeArraySize;
            if (mRearCamera) {
                uint32_t sensorW = fovRectIFEForWide.width;
                uint32_t sensorH = fovRectIFEForWide.height;
                // int32_t qcfaSensorSumFactor = inputCTB->cameraMetadata[i].qcfaSensorSumFactor;
                ...
                // We fix yuv output  size to  RDI size	without regard for ratio
                // Do scale & crop in IPE in JPEG pipeline
                mFrameParams.caminfo.i32AuxWidth_CropNoScale = 1600;
                mFrameParams.caminfo.i32AuxHeight_CropNoScale = 1200;
            }
        }
    }
    MLOGD(Mia2LogGroupPlugin,
    ...
    VendorMetadataParser::getVTagValue(pLogicalMetaData, BokehFNumber, &localMetadata);
    if (NULL != localMetadata) {
        memcpy(fNumber, static_cast<char *>(localMetadata), sizeof(fNumber));
        float fNumberApplied = atof(fNumber);
        mBlurLevel = findBlurLevelByCaptureType(fNumberApplied, false,
                                                ArcSoftBokeh1x); // inputCTB->bokehRole
    ...
    VendorMetadataParser::getVTagValue(pLogicalMetaData, BokehLight, &localMetadata);
    if (NULL != localMetadata) {
        lightMode = *static_cast<int *>(localMetadata);
    }
    switch (lightMode) {
    case 9:
    ...
    MLOGD(Mia2LogGroupPlugin, " [ARC_CTB] get mLightingMode: %d", mLightingMode);
    ...
    for (int i = 0; i < beautyFeatureCounts; i++) {
        if (mBeautyFeatureParams.beautyFeatureInfo[i].featureMask == true) {
            ...
            localMetadata = NULL;
            VendorMetadataParser::getVTagValue(pLogicalMetaData, tagStr, &localMetadata);
    ...
    VendorMetadataParser::getVTagValue(pLogicalMetaData, faceAnalyzeResult, &localMetadata);
    if (NULL != localMetadata) {
    ...
    VendorMetadataParser::getVTagValue(pLogicalMetaData, tagMultiCameraIds, &localMetadata);
    if (NULL != localMetadata) {
    ...
    /*
       localMetadata = NULL;
       VendorMetadataParser::getVTagValue(metaData, "xiaomi.device.orientation", &localMetadata);
       if (NULL != localMetadata)
       {
       int orientation = *static_cast<int*>(localMetadata);
       mFaceOrientation = (orientation + 90)%360;
       }
     */
    localMetadata = NULL;
    VendorMetadataParser::getTagValue(pLogicalMetaData, ANDROID_SENSOR_SENSITIVITY, &localMetadata);
    if (NULL != localMetadata) {
    ...
    VendorMetadataParser::getVTagValue(pLogicalMetaData, aecFrameControl, &localMetadata);
    if (NULL != localMetadata) {
    ...
    mIsAIIEEnabled = false;
    localMetadata = NULL;
    const char *AIIEenabled = "xiaomi.ai.asd.enabled";
    VendorMetadataParser::getVTagValue(pLogicalMetaData, AIIEenabled, &localMetadata);
    if (NULL != localMetadata) {
        mIsAIIEEnabled = *static_cast<uint8_t *>(localMetadata);
        MLOGD(Mia2LogGroupPlugin, " [ARC_CTB_AIIE] getMetaData AIIE enabled: %d", mIsAIIEEnabled);
    }
#endif
...
void ArcCapBokehPlugin::convertToArcFaceInfo(FDMetadataResults &fdMeta,
                                             ARC_DCR_FACE_PARAM &faceInfo, MIRECT &IFERect,
                                             int &nDataWidth, int &nDataHeight)
{
    ...
    for (int i = 0; i < fdMeta.numFaces && i < MAX_FACE_NUM; ++i) {
        memset(&rtOnInputData, 0, sizeof(MIRECT));
        mapRectToInputData(fdMeta.faceRect[i], IFERect, nDataWidth, nDataHeight, rtOnInputData);

        if (rtOnInputData.width != 0 && rtOnInputData.height != 0) {
            faceInfo.prtFaces[faceInfo.i32FacesNum].left = rtOnInputData.left;
            faceInfo.prtFaces[faceInfo.i32FacesNum].top = rtOnInputData.top;
            faceInfo.prtFaces[faceInfo.i32FacesNum].right =
                rtOnInputData.left + rtOnInputData.width - 1;
            faceInfo.prtFaces[faceInfo.i32FacesNum].bottom =
                rtOnInputData.top + rtOnInputData.height - 1;
            faceInfo.pi32FaceAngles[faceInfo.i32FacesNum] = getFaceDegree(mFaceOrientation);
            faceInfo.i32FacesNum++;
        } else {
            if (sDebugAbort) {
                raise(SIGABRT);
            }
            MLOGE(Mia2LogGroupPlugin, " [ARC_CTB] rtOnInputData is 0 and check it!!!");
        }
        ...
        MLOGD(Mia2LogGroupPlugin,
              ...
              fdMeta.faceRect[i].width, fdMeta.faceRect[i].height, getFaceDegree(mFaceOrientation));
    }
}
...
void ArcCapBokehPlugin::mapRectToInputData(MIRECT &rtSrcRect, MIRECT &rtIFERect, int &nDataWidth,
...
int ArcCapBokehPlugin::doProcess(std::vector<struct MiImageBuffer *> teleInputvector)
{
    ...
    ASVLOFFSCREEN InRelightImage = {0};
    ASVLOFFSCREEN OutRelightImage = {0};

    relight_thread_params relightParam;
    memset(&relightParam, 0, sizeof(relightParam));
    mRelightingHandle = NULL;
    ...
    if (mLightingMode > 0) {
        ...
        prepareRelightImage(pSrcImg, stInMainImage);

        relightParam.pSrcImg = pSrcImg;
        relightParam.orient = mFaceOrientation;
        relightParam.mode = mLightingMode;
        relightParam.m_phRelightingHandle = &mRelightingHandle;

        MLOGI(Mia2LogGroupPlugin,
              " [ARC_CTB] before init, give m_phRelightingHandle:%p, mRearCamera:%d",
              *relightParam.m_phRelightingHandle, mRearCamera);

        pthread_create(&tidRelighting, NULL, bokeh_relighting_thread_proc_prepare, &relightParam);

        MLOGI(Mia2LogGroupPlugin,
              " [ARC_CTB] right after init, got mRelightingHandle:%p, mRearCamera:%d",
    ...
    do {
        ...
        ARC_DCIR_SetShineParam(mCaptureBokeh, FILTER_SHINE_THRES, FILTER_SHINE_LEVEL);
    } while (false);
    ...
    if (mLightingMode > 0) {
        allocImage(&InRelightImage, &stInMainImage);
        pOutEffectImage = &InRelightImage;
    }
    ...
    if (true == mIsAIIEEnabled) {
        bokeh_AIIE_proc(mAIIEHandle, pInSrcImage);
    }
    ...
    if (mLightingMode > 0) {
        allocImage(&OutRelightImage, &stInMainImage);
        pInSrcImage = pOutEffectImage;
        pthread_join(tidRelighting, &tidRelightingExitStatus);
        processRelighting(&stOutSrcImage, &OutRelightImage, pInSrcImage);
        copyImage(&OutRelightImage, &stOutEffectImage);
    }

    pOutEffectImage = NULL;
    pInSrcImage = NULL;
    releaseImage(&InRelightImage);
    releaseImage(&OutRelightImage);
    releaseImage(&tmpHdrImage);
    releaseImage(&HdrcropImage);

    if (mRelightingHandle) {
        MLOGI(Mia2LogGroupPlugin,
              " [ARC_CTB] call ARC_PLI_Uninit with mRelightingHandle:%p, mRearCamera:%d",
              mRelightingHandle, mRearCamera);
...
int ArcCapBokehPlugin::processRelighting(ASVLOFFSCREEN *input, ASVLOFFSCREEN *output,
                                         ASVLOFFSCREEN *bokehImg)
{
    int nRet = MIA_RETURN_OK;
    double t0, t1, t2;
    ...
    MLOGI(Mia2LogGroupPlugin, " [ARC_CTB_RLT] mRelightingHandle: %p", mRelightingHandle);
    if (mRelightingHandle) {
        MIPLDEPTHINFO lDepthInfo = {0};
        MIPLDEPTHINFO *lDepthInfoTemp = &lDepthInfo;
        lDepthInfo.pBlurImage = &srcDokehImg;
        t1 = gettime();
        nRet = AILAB_PLI_Process(mRelightingHandle, lDepthInfoTemp, &srcImg, &LightParam, &dstImg);
        t2 = gettime();
        if (nRet != MOK) {
            MLOGE(Mia2LogGroupPlugin, " [ARC_CTB_RLT] AILAB_PLI_Process failed: %d", nRet);
            copyImage(bokehImg, output);
            nRet = MIA_RETURN_UNKNOWN_ERROR;
        }
        AILAB_PLI_Uninit(&mRelightingHandle);
        mRelightingHandle = NULL;
        MLOGI(Mia2LogGroupPlugin, " [ARC_CTB_RLT] AILAB_PLI_Process= %d: mode=%d, costime=%.2f",
              nRet, mLightingMode, t2 - t0);
    } else {
        nRet = MIA_RETURN_UNKNOWN_ERROR;
    }
    return nRet;
...
void ArcCapBokehPlugin::prepareRelightImage(MIIMAGEBUFFER &dstimage, ASVLOFFSCREEN &srtImage)
{
...
void ArcCapBokehPlugin::DumpInputData(ImageParams inputimage, const char *namePrefix, int Index)
{
    ...
    VendorMetadataParser::getTagValue(pMeta, ANDROID_CONTROL_AE_EXPOSURE_COMPENSATION, &pMetaData);
    if (NULL != pMetaData) {
        expValue = *static_cast<int32_t *>(pMetaData);
    } else {
        expValue = 0;
                                    ...
                                    const char *namePrefix)
{
    ...
    char fileName[256];
    memset(fileName, 0, sizeof(char) * 256);
    snprintf(fileName, sizeof(fileName), "%s%lld_%s_%d_%d.pdata", PluginUtils::getDumpPath(),
