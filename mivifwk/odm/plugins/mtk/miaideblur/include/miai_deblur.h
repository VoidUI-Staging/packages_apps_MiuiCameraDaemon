...
#define MIAI_SR_DEBLUR_VERSION "MIAI_SR_DEBLUR_2021_1102_1642"
...
union MIAIDeblurParams {
    ...
    struct _config
    {
        ...
        char
            txt_detect; //是否检测文字，默认不检测文字，可以通过：persist.vendor.camera.miaideblur.txt_detect
                        //设置
        char
            disable_deblur; //是否调过算法，默认执行deblur算法，可以通过：persist.vendor.camera.miaideblur.disable_deblur
                            //设置
        SceneType sceneType;
        void *flatSceneInfo;
        int tofDistance;
