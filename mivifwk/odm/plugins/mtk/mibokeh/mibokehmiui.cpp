...
void MiBokehMiui::initMiBokeh(unsigned int isPreview)
{
    if (isPreview) {
        ...
        mPreviewMiBokehInitThread = new std::thread([this, conf_loader] {
            ...
            mPreviewMiBokeh->Init(preview_conf, NULL);
            MLOGE(Mia2LogGroupPlugin, "MiBokehMiui.cpp, preview init, costTime=%.2f",
        ...
        mPostMiBokehInitThread = new std::thread([this, conf_loader] {
            ...
            mPostMiBokeh->ChangeModel(bokeh::BokehModelShape::MODEL_512_512_QIANZHI_GPU_3,
                                      post_conf);
            mPostMiBokeh->Init(post_conf, NULL);
            MLOGE(Mia2LogGroupPlugin, "MiBokehMiui.cpp, snapshot init with depth, costTime=%.2f",
                          ...
                          float fNumberApplied, int sceneflag)
{
    ...
    if (isPreview) {
        if (!mPreviewInitFinish) {
            PluginUtils::miCopyBuffer(output, miBufMain);
            MLOGE(
                Mia2LogGroupPlugin,
                "MiBokehMiui.cpp process preview not ready width=%d, orientation=%d, costTime=%.2f",
                miBufMain->width, rotate, PluginUtils::nowMSec() - startTime);
            return;
        }
        ...
        //     miBufMain->stride[1], miBufMain->plane[3] - miBufMain->plane[2],
        //     miBufMain->stride[2], miBufMain->stride[3], PluginUtils::nowMSec() - startTime);
    } else {
        if (mPostMiBokehInitThread && mPostMiBokehInitThread->joinable()) {
            mPostMiBokehInitThread->join();
        }
