...
S32 NodeMetaData::GetLdcLevel(camera_metadata_t *mdata, U64 frameNum, U32 *pLdcLevel)
{
...
S32 NodeMetaData::GetCropRegion(camera_metadata_t *mdata, U64 frameNum, MIRECT *pCropRegion,
                                uint32_t mode = StreamConfigModeSAT)
{
    ...
    //    ret = VendorMetadataParser::getVTagValue(mdata, "xiaomi.superResolution.cropRegionMtk",
    //    &pData);
    //}
...
S32 NodeMetaData::GetCropRegionRef(camera_metadata_t *mdata, U64 frameNum, MIRECT *pCropRegionRef)
{
...
S32 NodeMetaData::GetActiveArraySize(camera_metadata_t *mdata, MIRECT *pActiveArraySize)
{
...
S32 NodeMetaData::isFastShot(camera_metadata_t *mdata, bool *isFastShot)
{
    // dont process aildc on cshot
    S32 ret;
    void *pData = NULL;
    *isFastShot = false;
    VendorMetadataParser::getVTagValue(mdata, "com.mediatek.cshotfeature.capture", &pData);
    if (NULL != pData) {
        *isFastShot = ((*static_cast<S32 *>(pData)) > 0) ? true : false;
        DEV_LOGI("get cshotenable = %d", *isFastShot);
        ret = RET_OK;
    } else {
        DEV_LOGI("get tag com.mediatek.cshotfeature.capture fail");
        ret = RET_ERR;
    }
    return ret;
}

S32 NodeMetaData::GetFaceInfo(camera_metadata_t *mdata, U64 frameNum, NODE_METADATA_MODE mode,
                              ...
                              MIRECT sensorSize)
{
    ...
    float widthRheight = (float)sensorSize.width / sensorSize.height;
    ...
    if (ratioWidth > (ratioHeight + AspectRatioTolerance)) {
        ...
        gapWidth = ((pFaceInfo->imgHeight * widthRheight) - pFaceInfo->imgWidth) / 2;
...
S32 NodeMetaData::GetQcfa(camera_metadata_t *mdata, U64 frameNum, U32 *pQcfa)
{
...
S32 NodeMetaData::GetZoomRatio(camera_metadata_t *mdata, float *pZoomRatio)
{
    S32 ret;
    void *pData = NULL;
    ...
    *pZoomRatio = *reinterpret_cast<float *>(pData);
    DEV_LOGI("Get ZoomRatio %f  %d", *pZoomRatio, ret);
    return RET_OK;
}
