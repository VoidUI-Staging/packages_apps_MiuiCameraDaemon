...
typedef enum {
    ...
    alAILDC_PARA_EDIT_TYPE_BACKGROUND_DISTORTION_TOLERANCE =
        2, /*SIZE_FLOAT, 1.0~180.0, <= correction strength*/
...
typedef enum {
    ...
    alAILDC_DEBUG_FLAG_DUMP_INPUT_DATA =
        0x00000100, /*Dump input image, tuning data, model data, external mask and parameters*/
 ...
 */
alAILDC_ERR_CODE alAILDC_Buffer_Init(int a_dID, int a_dInImgW, int a_dInImgH, int a_dOutWidth,
                                     int a_dOutHeight, int a_dStrideWidth, int a_dStrideHeight,
                                     int a_dOutStrideWidth, int a_dOutStrideHeight,
                                     alAILDC_IMG_FORMAT a_nImgFormat, unsigned char *a_pLDCBuffer,
 ...
 */
alAILDC_ERR_CODE alAILDC_Buffer_Init_v2(int a_dID, int a_dInImgW, int a_dInImgH, int a_dOutWidth,
                                        int a_dOutHeight, int a_dStrideWidth, int a_dStrideHeight,
                                        int a_dOutStrideWidth, int a_dOutStrideHeight,
                                        ...
                                        unsigned char *a_pModelBuffer,
                                        unsigned int a_udModelBufferSize);

/* alAILDC_Buffer_Run
 * To run LDC and get result images.
 * a_dID: instance id
 * a_pucInImage_Y, a_pucInImage_UV: Input Image buffer the format in which was set when
 * initializaiton. a_pucOutImaghe_Y, a_pucOutImage_UV: Output reslut image buffer. the format is
 * similar to input. a_dInputRotate: set 0 if input image is not rotated. 1 for rotated input image.
 * a_pfMvpMatrix: set nullptr if the output result is not expected to rotate. Otherwise, input the
 * matrix. a_ptInZoomWOI: Input the WOI of current Zoom level. refers to woi structure alAILDC_RECT.
 ...
 *                 Set flag=1 if expected to get lastest table in this run. And will get LDC table
 * buffer point.
 */
alAILDC_ERR_CODE alAILDC_Buffer_Run(int a_dID, unsigned char *a_pucInImage_Y,
                                    unsigned char *a_pucInImage_UV, unsigned char *a_pucOutImage_Y,
                                    unsigned char *a_pucOutImage_UV, int a_dInputRotate,
                                    float *a_pfMvpMatrix, alAILDC_RECT *a_ptInZoomWOI,
                                    alAILDC_FACE_INFO *a_ptInFaceInfo,
 ...
 */
alAILDC_ERR_CODE alAILDC_Set_SubjectMask(int a_dID, unsigned char *a_pucMaskImage_Y,
                                         int a_dMaskWidth, int a_dMaskHeight);
 ...
 */
alAILDC_ERR_CODE alAILDC_Map_Location(int a_dID, alAILDC_ROI_MAPPING_T *a_ptInOutRoi,
                                      alAILDC_MAP_ROI_FLAG_T a_nflag);
...
*/
alAILDC_ERR_CODE alAILDC_Set_TuningParameters(int a_dID, alAILDC_SET_TUNE_PARA a_dFlag,
                                              void *a_Value);
