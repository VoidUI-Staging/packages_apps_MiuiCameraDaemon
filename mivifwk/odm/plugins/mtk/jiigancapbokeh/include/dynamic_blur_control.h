 ...
 * Copied from node/arcsoft/common/bokeh/inc/dynamic_blur_control.h
 **/
...
#define DEFAULT_ANC_FNUMBER 4.0f
...
static AncFNumberLevel AncCaptureFNumberLevel[] = {
    {1.0f, 1.0f},
    {1.1f, 1.0f},
    {1.2f, 1.4f},
    {1.4f, 1.4f},
...
static float convertToAncFNumber(float fnum)
{
    float ancFNumber = 0;
    uint32_t size = 0;
    bool isFound = false;

    size = sizeof(AncCaptureFNumberLevel) / sizeof(AncFNumberLevel);
    for(uint32_t i = 0; i < size; i++)
    {
        if (fnum == AncCaptureFNumberLevel[i].f_number)
        {
            ancFNumber = AncCaptureFNumberLevel[i].anc_fnumber;
            isFound = true;
            break;
        }
    }
    if (0 == ancFNumber && false == isFound)
    {
            ancFNumber = DEFAULT_ANC_FNUMBER;
    }
    return ancFNumber;
}
