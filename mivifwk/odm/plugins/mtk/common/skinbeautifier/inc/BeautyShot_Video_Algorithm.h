...
public:
     ...
     * [IN] LevelVersion: different LevelVersion for different level of phone,
     *      LevelVersion = 0 for Low-end phone,
     *      LevelVersion = 1 for Middle-end phone,
     *      LevelVersion = 2 for High-end phone.
     * [IN] nPlatFromType: different skinsoften type of different PlatForm.
     *      nPlatFromType = 0 for ABS_VIDEO_PLATFORM_TYPE_CPU,
     *      nPlatFromType = 1 for ABS_VIDEO_PLATFORM_TYPE_HVX,
     * [IN] nUseOnebuffer: default is 0;if use bUseOnebuffer > 0, only support one buffer and 2
     * threads
     */
    virtual MRESULT Init(const MInt32 nInterfaceVersion, MInt32 LevelVersion, MInt32 nPlatFromType,
                         MInt32 nUseOnebuffer) = 0;
     ...
     * [IN] pFacesIn: The detected faces info of input image. If it is MNull, SDK will detect faces
     * inside. [OUT] pFacesOut: The output faces info. If it is MNull, SDK does not output faces
     * info. [IN] nFaceOrientation: Face Orientation. [0, 90, 180, 270].
     */
    virtual MRESULT Process(LPASVLOFFSCREEN pImgIn, LPASVLOFFSCREEN pImgOut, ABS_TFaces *pFacesIn,
                            ABS_TFaces *pFacesOut, MInt32 nFaceOrientation) = 0;
};
 ...
 *                and shall be released by calling (*ppalgorithm)->Release() when it is not used
 * anymore. Return: MOK is succeeded, others are failed.
 */
#ifdef __cplusplus
extern "C" {
#endif
