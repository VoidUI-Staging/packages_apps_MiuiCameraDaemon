...
#define MACE_PATH   "/sdcard/Android/data/com.android.camera/files"
...
void MiAiSuperLowlight::init()
{
    ...
    if (m_hSuperLowlightEngine) {
        ...
        MIAI_SN_Uninit(&m_hSuperLowlightEngine);
        m_hSuperLowlightEngine = MNull;
    }

    ALOGD("[MIAI]: superlowlight proc version=%s\n", MIAI_SN_GetVersion()->Version);

    mAlgoInitRet = MIAI_SN_Init(HARDCODE_CAMERAID_FRONT, &m_hSuperLowlightEngine);
    if (mAlgoInitRet != MOK) {
...
{
    int ret = MIAI_SN_Uninit(&m_hSuperLowlightEngine);
    if (ret != MOK)
...
void MiAiSuperLowlight::process(struct MiImageBuffer *input, struct MiImageBuffer *output,
                                ...
                                LPMIAI_LLHDR_AEINFO pAeInfo, bool IsFlashOn, int orient)
{
    (void *)output;
    MRESULT ret;
    MIAI_SN_INPUTINFO m_InputFrames;
    MIAI_SN_PARAM m_Param;
    ...
    ALOGD("[MIAI]: superlowlight init ret=%ld", ret);

    memset(&m_InputFrames, 0, sizeof(m_InputFrames));
    memset(&m_Param, 0, sizeof(m_Param));

    m_InputFrames.i32ImgNum = input_num;
    m_InputFrames.i32CameraState = MIAI_SN_CAMERA_STATE_HAND;
    ...
    property_get("persist.vendor.algoengine.supernight.bypass", prop, "0");
    int32_t m_passby = (int32_t)atoi(prop);
    ALOGI("[MIAI]: m_passby = %d", m_passby);

    ALOGI("[MIAI]: totalGain=%.2f, brightness=%d, contrast=%d\n", gain, m_Param.i32CurveBrightness,
    ...
    int32_t i32IsFlashOn = 0;
    if (IsFlashOn) {
        i32IsFlashOn = 1;
    }
    ret = MIAI_SN_Process(m_hSuperLowlightEngine, gain, &m_InputFrames, &pOutputImg, &m_Param, NULL,
                          orient, pAeInfo, i32IsFlashOn, MACE_PATH, m_passby);
