...
struct SceneConf {
  SceneConf() : scene_category_(ClassifyCategory::INVALID_BY_MANUAL), scene_sequence_(0),
                filter_strength_(0.0f) {}

  SceneConf(ClassifyCategory scene_category, int scene_sequence, float filter_strength)
          : scene_category_(scene_category), scene_sequence_(scene_sequence),
...
struct BlurConf {
  BlurConf() : algorithm_(BlurAlgorithm::FAST), spot_shape_(BlurSpotShape::CIRCLE), max_radius_(0),
               radius_(0), gamma_(0.0f), gamma_thresh_(0.0f), cut_(-1),
               extension1_(-1.0f), extension2_(-1.0f), extension3_(-1.0f) {}

  BlurConf(BlurAlgorithm algorithm, BlurSpotShape spot_shape, int max_radius, int radius,
           float gamma, float gamma_thresh, int cut, float extension1, float extension2, float extension3)
      : algorithm_(algorithm), spot_shape_(spot_shape), max_radius_(max_radius), radius_(radius), gamma_(gamma),
        gamma_thresh_(gamma_thresh), cut_(cut), extension1_(extension1), extension2_(extension2), extension3_(extension3) {}
...
enum class BokehModelShape {
  ...
  MODEL_184_256_V5 = 42,
  MODEL_384_384_V1 = 43,
  MODEL_384_384_V2 = 44, //front resize
  MODEL_512_512_V5 = 45,
  MODEL_512_512_QIANZHI_GPU_3 = 46,
  MODEL_384_384_QIANZHI_14 = 47,
  MODEL_512_512_QIANZHI_27 = 48,
  MODEL_512_512_DEPTH_ESTIMATION = 49,
  ...
  MODEL_768_768_BACK_APU_V1 = 56,
  MODEL_384_384_FRONT_GPU_6x = 57,
  MODEL_512_512_FRONT_GPU_8x = 58,
  MODEL_512_512_BACK_V2 = 59,
  MODEL_768_768_MFKT_V1 = 60,
  MODEL_512_512_BACK_DXO = 61,
  MODEL_384_384_BACK_TWICESEGMENT_FIRST = 62,
  MODEL_512_512_BACK_TWICESEGMENT_SECOND = 63,
  MODEL_768_768_BACK_GPU = 64,
...
struct SegmentConf {
  SegmentConf() : photo_orient_(Orientation::NORMAL), photo_flip_(0), async_seg_peroid_ms_(0),
                  runtime_(SegmentRuntime::GPU), size_seg_({0, 0}),
                  mean_r_(0.0f), mean_g_(0.0f), mean_b_(0.0f), divider_(0.0f), gf_radius_(0),
                  gf_eps_(0.0f), model_(BokehModelShape::MODEL_EMPTY), with_det_(true), with_refine_(true),
                  has_portrait_th_(0.5f), has_portrait_area_(32*32) {}

  SegmentConf(Orientation photo_orient, bool photo_flip, int async_seg_peroid_ms,
              SegmentRuntime runtime,
              Rect size_seg, float mean_r, float mean_g, float mean_b, float divider, float depth_divider_r, float depth_divider_g, float depth_divider_b, int gf_radius,
              float gf_eps, BokehModelShape model, bool with_det, bool with_refine, float portrait_th, int portrait_area) : photo_orient_(photo_orient),
                                                     photo_flip_(photo_flip),
                                                     async_seg_peroid_ms_(async_seg_peroid_ms),
                                                     runtime_(runtime), size_seg_(size_seg),
                                                     mean_r_(mean_r), mean_g_(mean_g),
                                                     mean_b_(mean_b),
                                                     divider_(divider),
                                                     divider_r_(depth_divider_r), divider_g_(depth_divider_g), divider_b_(depth_divider_b),
                                                     gf_radius_(gf_radius),
                                                     gf_eps_(gf_eps), model_(model),with_det_(with_det), with_refine_(with_refine),
                                                     has_portrait_th_(portrait_th), has_portrait_area_(portrait_area) {}
  ...
  float divider_r_;                    //divider of channel （深度估计时，每个通道的方差不一样）
  float divider_g_;
  float divider_b_;
...
struct BokehConf {
  BokehConf() : pixel_format_(PixelFormat::NV21), uv_offset_(0), stride_(0),
                size_max_portrait_blur_(Rect()), size_in_(Rect()),
                blur_conf_(BlurConf()), night_blur_conf_(BlurConf()),
                blur_version(BokehBlurVersion::BLUR_VERSION_EMPTY), scene_conf_(SceneConf()),
  ...
  BokehConf(PixelFormat pixel_format, int uv_offset, int stride, Rect size_max_portrait_blur,
            ...
            const char conf_name[100]) : pixel_format_(pixel_format), uv_offset_(uv_offset), stride_(stride),
                                    size_max_portrait_blur_(size_max_portrait_blur), size_in_(size_in),
                                    blur_conf_(blur_conf), night_blur_conf_(night_blur_conf),
                                    blur_version(blur_version),
                                    scene_conf_(scene_conf),
                                    seg_conf_(seg_conf),
                                    reserve1_(reserve1), reserve2_(reserve2), reserve3_(reserve3), reserve4_(reserve4), reserve5_(reserve5)
...
enum VideoMode {
  ...
  MODE_DARK_KEEP = 4, //暗角留色
  MODE_EXPAND_BOKEH = 5, //发散虚化
  MODE_SHRINK_BOKEH = 6, //收缩虚化
  MODE_ROTATE_BOKEH = 7, //旋转虚化
  MODE_KEEP_AND_EXPAND_BOKEH = 8, //留色和发散虚化
  MODE_KEEP_AND_SHRINK_BOKEH = 9, //留色和收缩虚化
  MODE_KEEP_AND_ROTATE_BOKEH = 10, //留色和旋转虚化
};
...
typedef struct stylize_param{
  int isPersonIn;     //if there is person, 1; else, 0
  int bokeh_factor;   //measure the degree of bokeh
