...
#define MIBOKEH_SO_VERSION "2021_1213_1646"
...
class MiBokehFactory {
 public:
  ...
  // 创建 BokehConfLoader 实例，加载 filename 指定的配置文件并返回。返回的 BokehConfLoader 实例不需要释放。
...
class MiBokeh {
 public:
  ...
  // 在输入图像的分辨率在 init 时没有传入的情况下，在调用 Bokeh 函数前调用，更新输入分辨率的图片朝向
  // 输入格式为 NV21，要求 y plane 和 uv plane 连续且 stride 相等，uv_offset 为两者起始地址相差的字节数
  virtual int UpdateSizeIn(int width, int height, int rotate_angle, int uv_offset, int stride, bool flip) = 0;
  ...
  virtual void BlurAndBlendNv21(int width, int height, int uv_offset, int stride, uint8_t* to_img) = 0;

  // 功能与 BlurAndBlendNv21 类似，但使用 Prepare 的图像虚化作为背景，to_img 用于提取前景。
  // 只能用于 NV21 格式，输入分辨率可以与 Prepare 的输入不同。to_img 是用于前景的输入，也保存虚化后的结果。
  ...
  // 与 Blend 类似，要求分辨率与 Prepare 输入图像相同，当 Prepare 输入为 NV21，img / to_img 格式为 rgb 时使用。
  ...
  virtual void GetDepthImage(void *depthImage, uint8_t* src, int width, int height, int rotate_angle, int uv_offset, int stride, bool flip) = 0;
  ...
  virtual void Bokeh(void *pDisparityData, MiBufferPtr pMainImg, float strength, int category, MiBufferPtr pDstImg) = 0;
...
void dumpYuvBuffer(const char* suffix, uint8_t* img, int width, int height, int uv_offset, int stride, bool flip, int orientation, bool switchRB);
void dumpYuvBuffer(const char* prefix, const char* suffix, uint8_t* img, int width, int height, int uv_offset, int stride, bool flip, int orientation, bool switchRB);

//id photo
void pushData(int width, int height, int rotate_angle, int uv_offset, int stride, bool flip, uint8_t* img);
