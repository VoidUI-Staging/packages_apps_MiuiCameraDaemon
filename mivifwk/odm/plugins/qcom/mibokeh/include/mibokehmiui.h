...
public:
    ...
    void process(struct MiImageBuffer *input, struct MiImageBuffer *output, unsigned int isPreview, int rotate, int64_t timestamp, float fNumberApplied, int sceneflag);
    MInt32  process(struct MiImageBuffer *input, struct MiImageBuffer *output, MUInt8 *outputDepth, unsigned int isPreview,int rotate, int64_t timestamp, unsigned int currentFlip, float fNumberApplied, int sceneflag);//
};
