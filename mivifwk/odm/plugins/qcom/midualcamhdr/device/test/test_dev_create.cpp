...
#endif
    m_Result |= Device->semaphore->Create(&m_create_Id1, MARK("m_create_Id1"));
    DEV_IF_LOGE((m_create_Id1 == m_create_Id2), "CREATE ID LOCK TEST ERR!!!!!!!!!!!!!!!!!%ld", m_create_Id1);
    if (m_create_Id1 == m_create_Id2) {
        m_Result |= RET_ERR;
    }
//    m_Result |= Device->system->SleepUs(1);
    m_Result |= Device->semaphore->Destroy(&m_create_Id1);
#if (USE_MUTEX_LOCK==1)
...
void run_Create2() {
    ...
    m_Result |= Device->mutex->Lock(m_mutex_Id);
#endif
    m_Result |= Device->semaphore->Create(&m_create_Id2, MARK("m_create_Id2"));
    DEV_IF_LOGE((m_create_Id1 == m_create_Id2), "CREATE ID LOCK TEST ERR!!!!!!!!!!!!!!!!!%ld", m_create_Id2);
    if (m_create_Id1 == m_create_Id2) {
        m_Result |= RET_ERR;
    }
//    m_Result |= Device->system->SleepUs(3);
    m_Result |= Device->semaphore->Destroy(&m_create_Id2);
#if (USE_MUTEX_LOCK==1)
    m_Result |= Device->mutex->Unlock(m_mutex_Id);
#endif
}
