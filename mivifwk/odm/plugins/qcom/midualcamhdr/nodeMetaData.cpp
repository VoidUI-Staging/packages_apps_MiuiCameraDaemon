...
S32 NodeMetaData::GetFaceRect(camera_metadata_t *mdata, DEV_IMAGE_BUF *inputMain,
                              MIAI_HDR_FACEINFO *face_info)
{
    DEV_IF_LOGE_RETURN_RET((inputMain == NULL || face_info == NULL), RET_ERR_ARG, "arg err");

    memset(face_info, 0, sizeof(MIAI_HDR_FACEINFO));

    S32 ret = RET_OK;
    U32 rotateAngle = 0;
    ret = GetRotateAngle(mdata, &rotateAngle);
    DEV_IF_LOGE_RETURN_RET(ret != RET_OK, ret, "GetMetadata err");
