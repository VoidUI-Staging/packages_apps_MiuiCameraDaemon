...
typedef struct _tag_ARC_DCIR_ENHANCE_PARAM {
	MPOINT		ptFocus;		                         // [in]  The focus point to decide which region should be clear
	MInt32      i32Intensity; 		                     //[in] set the intensity of MF noise removal Range is [0,100], default is 40.
	MInt32      i32LightIntensity;                   	 //[in] set the intensity of brightness enhancement Range is [0,100], default is 0.
...
ARCDCIR_API MRESULT  ARC_DCIR_SetCameraRoll(
	MHandle				        hEngine,			     // [in]  The algorithm engine
	MInt32                      i32Degree                // [in] Camera roll degree
	);
ARCDCIR_API MRESULT  ARC_DCIR_SetFullWeakBlur(
		MHandle				        hEngine,		 // [in]  The algorithm engine
...
//lowlight
ARCDCIR_API MRESULT  ARC_DCIR_Enhancement(
		MHandle				        hEngine,			     // [in]  The algorithm engine
	    LPARC_DCIR_EHINPUTINFO	    pMainImgs,			     // [in]  The offscreen of source images
		LPASVLOFFSCREEN		        pDstMainImg,		     // [out] The offscreen of enhance image
		MVoid						*pDisparityData,		 // [in]  The data of disparity data
		MInt32						i32DisparityDataSize,	 // [in]  The size of disparity data
		LPARC_DCIR_ENHANCE_PARAM	pEhanParam			     // [in]  The parameters for algorithm engine
);
...
//For HDR
//HDR error code
#define ARC_DCIR_HDR_ERR_INVALID_HANDLE           	 	(MERR_BASIC_BASE+20)
#define ARC_DCIR_HDR_ERR_INVALID_PARAM               	(MERR_BASIC_BASE+21)
#define ARC_DCIR_HDR_ERR_ALIGN_FAIL                	(MERR_BASIC_BASE+22)
#define ARC_DCIR_HDR_ERR_INVALID_EXPOSURE            	(MERR_BASIC_BASE+23)

//for HDR params
typedef struct _tag_ARC_DCIR_HDR_PARAM
{
	...
	MInt32 i32Contrast;                             	//[in] Adjustable parameter for contrast, the range is 0~+100

} ARC_DCIR_HDR_PARAM, *LPARC_DCIR_HDR_PARAM;

//for HDR defualt params
ARCDCIR_API MRESULT ARC_DCIR_GetDefaultParam_HDR(     //return MOK if success, otherwise fail
		MHandle hHandle,							//[in] The camera type,ARC_DCBP_CAMERA_REAR or ARC_DCBP_CAMERA_FRONT
		LPARC_DCIR_HDR_PARAM pstParam        	    //[in/out] The default param for sdk
);
//HDR process
//currently support 0 ev + -2 Ev
ARCDCIR_API MRESULT ARC_DCIR_HDRProcess(              //return MOK if success, otherwise fail
		MHandle hHandle,        						//[in] The algorithm engine
		LPARC_DCIR_EHINPUTINFO pMainImgs,        		//[in] Pointer to input source image data
		MVoid *pDisparityData,                          //[in] The data of disparity data
		MInt32 i32DisparityDataSize,                    //[in] The size of disparity data
		LPARC_DCIR_HDR_PARAM pstParam,        			//[in] The enhance parameters for algorithm engine
		LPARC_DCR_FACE_PARAM pFaceInfo,
		LPASVLOFFSCREEN pDstImg,           		    //[out]The offscreen of result image
		MRECT *pRetCropRect							//[out]The return crop rect

);
ARCDCIR_API MRESULT ARC_DCIR_CropResizeImg(					// return MOK if success, otherwise fail
		MHandle			            	hHandle ,   				    // [in]  The algorithm engine
		LPASVLOFFSCREEN		            pImg,		            // [in] The offscreen of enhance image
		MRECT*                          pHDRRect,                      // [in] HDR Crop region
		LPASVLOFFSCREEN		            pDestImg		               // [in/Out] The offscreen of enhance image
);
