 ...
 */
// clang-format off
#ifndef __INTSENSE_UTILS_H__
#define __INTSENSE_UTILS_H__
...
static void __attribute__((unused)) DrawPoint(WAOFFSCREEN* pOffscreen, MPOINT xPoint, MInt32 nPointWidth, MInt32 iType)
{
    if(MNull == pOffscreen)
    {
        return;
    ...
    if(pOffscreen->u32PixelArrayFormat == wa::Image::IMAGETYPE_NV21 || wa::Image::IMAGETYPE_NV12 == pOffscreen->u32PixelArrayFormat)
    {
...
static void __attribute__((unused)) DrawRect(LPWAOFFSCREEN pOffscreen, MRECT RectDraw, MInt32 nLineWidth, MInt32 iType)
{
    if(MNull == pOffscreen)
    {
        return;
    ...
    if (pOffscreen->u32PixelArrayFormat == wa::Image::IMAGETYPE_YUYV)
    {
        nLeft=(nLeft+1)/2*2;
        nRight = (nRight+1)/2*2;
    }
...
static void __attribute__((unused))  DrawRectROI(LPWAOFFSCREEN pOffscreen, MIRECT rt, MInt32 nLineWidth, MInt32 iType)
{
    int l = rt.left;
    int t = rt.top;
