...
#define MEMCP_PATH             "/vendor/etc/camera/dof_mecp.bin"
#define MEMCP_PATH_FOR_NOCALIB "/vendor/etc/camera/dof_mecp.bin"
#elif defined(RENOIR_CAM)
#define MEMCP_PATH             "/vendor/etc/camera/com.xiaomi_k9_mecp.bin"
#define MEMCP_PATH_FOR_NOCALIB "/vendor/etc/camera/com.xiaomi_k9_mecp_for_nocali.bin"
#endif

#define MODEL_PATH             "/vendor/etc/camera/model/dof_model"

#define CALIBRATIONDATA_LEN 4096

#define DOF_ROTATION 180
...
int MiaNodeIntSenseCapBokeh::init(uint32_t cameraId)
{
    ...
    mMainExposureTime = 70000000;
    mAuxExposureTime = 20000000;
    mMainIsoSensitivity = 280;
    mAuxIsoSensitivity = 900;
    mMainGain = 1;
    mAuxGain = 1;
    ...
    mCameraId = CAM_COMBMODE_REAR_BOKEH_WU;
    //#endif
...
int MiaNodeIntSenseCapBokeh::iniEngine()
{
    ...
    MLOGI(Mia2LogGroupPlugin, "mForceNonCalibAlgo = %d", mForceNonCalibAlgo);
...
#define SHOW_HALF_HALF 3
    ...
    MLOGI(Mia2LogGroupPlugin, "MiaNodeIntSenseCapBokeh snapshot process  costTime = %.2f",
                                            ...
                                            camera_metadata_t *metaData)
{
    ...
    if (0 == strcmp(mModuleInfo, "")) // only set once
    {
        property_get("persist.vendor.camera.mi.module.info.bokeh", mModuleInfo, "");
        if (0 == strcmp(mModuleInfo, "")) {
            strcpy(mModuleInfo, "L10_IMX686_OV08856_S1");
            MLOGD(Mia2LogGroupPlugin, " GetModuleInfo  failed. Set default value = %s",
                  mModuleInfo);
        } else {
            MLOGD(Mia2LogGroupPlugin, " ModuleInfo = %s", mModuleInfo);
        }
    }
    ...
    /* xiaomi.device.orientation
        pMetaData = NULL;
        MetadataUtils::getInstance()->getVTagValue(metaData, Device, &pMetaData);
        if (NULL != pMetaData)
        {
    ...
    */
    ...
    for (uint8_t i = 0; i < MAX_LINKED_SESSIONS; i++) {
        MLOGD(Mia2LogGroupPlugin,
              "setFrameParams cameraMetadata[%d] masterCameraId %d cameraId %d "
              "activeArraySize(t,l,w,h): [%d, %d, %d ,%d] "
              "fovRectIFE(t,l,w,h): [%d, %d, %d ,%d], sensorIFEAppliedCrop(t,l,w,h): [%d, %d, %d "
              ",%d]",
              i, pInputCTB->cameraMetadata[i].masterCameraId, pInputCTB->cameraMetadata[i].cameraId,
              ...
              pInputCTB->cameraMetadata[i].sensorIFEAppliedCrop.top,
              pInputCTB->cameraMetadata[i].sensorIFEAppliedCrop.left,
              pInputCTB->cameraMetadata[i].sensorIFEAppliedCrop.width,
              pInputCTB->cameraMetadata[i].sensorIFEAppliedCrop.height);

        if (pInputCTB->cameraMetadata[i].masterCameraId == pInputCTB->cameraMetadata[i].cameraId) {
            mMainExposureTime = pInputCTB->cameraMetadata[i].exposureTime;
            mMainIsoSensitivity = pInputCTB->cameraMetadata[i].isoSensitivity;
            // mMainGain = pInputCTB->cameraMetadata[i].totalGain;
            ...
            MLOGD(Mia2LogGroupPlugin, " tele facenum = %d",
            ...
            for (int nFaceIndex = 0; nFaceIndex < pInputCTB->cameraMetadata[i].fdMetadata.numFaces;
                 ++nFaceIndex) {
                MLOGD(Mia2LogGroupPlugin, " facerect[%d](l,t,w,h)[%d,%d,%d,%d]", nFaceIndex,
                      pInputCTB->cameraMetadata[i].fdMetadata.faceRect[nFaceIndex].left,
            ...
            // for Face info
            convertToWaFaceInfo(pInputCTB->cameraMetadata[i].fdMetadata, mBokehParam.faceParam);
        } else {
            mAuxExposureTime = pInputCTB->cameraMetadata[i].exposureTime;
            mAuxIsoSensitivity = pInputCTB->cameraMetadata[i].isoSensitivity;
            // mAuxGain = pInputCTB->cameraMetadata[i].totalGain;
    ...
    for (int i = 0; i < beautyFeatureCounts; i++) {
        if (mBeautyFeatureParams.beautyFeatureInfo[i].featureMask == true) {
            ...
            if (pMetaData != NULL) {
                ...
                MLOGE_BF("Get Metadata Failed! TagName: %s", tagStr);
            }
        }
    }
    ...
    pMetaData = NULL;
    const char *multiCameraRole = "com.qti.chi.multicamerainfo.MultiCameraIdRole";
    VendorMetadataParser::getVTagValue(metaData, multiCameraRole, &pMetaData);
    if (NULL != pMetaData) {
        MultiCameraIds inputMetadata = *static_cast<MultiCameraIds *>(pMetaData);
    ...
    if (NULL != pMetaData) {
        ...
        MLOGD(Mia2LogGroupPlugin, "getMetaData lux_index: %f", lux_index);
    }
                                                  ...
                                                  WA_FACE_PARAM &faceinfo)
{
    ...
    for (int i = 0; i < fdMeta.numFaces && i < MAX_FACE_NUM; ++i) {
        ...
        mapRectToInputData(fdMeta.faceRect[i], mActiveArraySizeTele, mInputDataWidth,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// MiaNodeIntSenseCapBokeh::doProcess
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int MiaNodeIntSenseCapBokeh::doProcess()
{
    ...
    do {
        ...
        Image depthImage;
        // get depth
        mainImage = Image(stTeleImage.aWidth, stTeleImage.aHeight, stTeleImage.ppu8Plane[0],
                          stTeleImage.ppu8Plane[1], NULL, stTeleImage.u32PixelArrayFormat,
                          Image::DT_8U, wa::Image::ROTATION_180);
        mainImage.setImageWH(stTeleImage.i32Width, stTeleImage.i32Height);
        mainImage.setExifExpTime(mMainExposureTime);
        mainImage.setExifIso(mMainIsoSensitivity);
        mainImage.setExifAeGain(mMainGain);
        // mMainImage.setExifModuleInfo(mModuleInfo);
        MLOGD(Mia2LogGroupPlugin,
              " stTeleImage.ExpTime = %lu, stTeleImage.Iso = %d, stTeleImage.Gain = %f",
              mMainExposureTime, mMainIsoSensitivity, mMainGain);

        auxImage = Image(stWideImage.aWidth, stWideImage.aHeight, stWideImage.ppu8Plane[0],
        ...
        auxImage.setExifExpTime(mAuxExposureTime);
        auxImage.setExifIso(mAuxIsoSensitivity);
        auxImage.setExifAeGain(mAuxGain);
        // mAuxImage.setExifModuleInfo(mModuleInfo);
        MLOGD(Mia2LogGroupPlugin,
              " stWideImage.ExpTime = %lu, stWideImage.Iso = %d, stWideImage.Gain = %f",
              mAuxExposureTime, mAuxIsoSensitivity, mAuxGain);
        ...
        if (ret == MIA_RETURN_OK) { // depth run successfull and no memory issue
            // do bokeh
            tmpMainImage =
                Image(tmpTeleImage.aWidth, tmpTeleImage.aHeight, tmpTeleImage.ppu8Plane[0],
            ...
            tmpMainImage.setExifExpTime(mMainExposureTime);
            tmpMainImage.setExifIso(mMainIsoSensitivity);
            tmpMainImage.setExifAeGain(mMainGain);
            mRefocus->setImages(tmpMainImage, depthImage);

            bokehImage = Image(stDestImage.aWidth, stDestImage.aHeight, stDestImage.ppu8Plane[0],
                               stDestImage.ppu8Plane[1], NULL, stDestImage.u32PixelArrayFormat,
                               Image::DT_8U, wa::Image::ROTATION_180);
            bokehImage.setImageWH(stDestImage.i32Width, stDestImage.i32Height);

            waFNumber = convertToAncFNumber(mFNum);
        ...
        if (mRelightingHandle && mLightingMode > 0) {
            result = processRelighting(tmpTeleImage, tmpArlDstImage, stDestImage, maskImage);
            if (result == 0) {                            // success
    ...
    MLOGI(Mia2LogGroupPlugin, "Total costtime = %.2f X", PluginUtils::nowMSec() - t0);
...
void MiaNodeIntSenseCapBokeh::copyWaImage(WAOFFSCREEN *pDstImage, WAOFFSCREEN *pSrcImage)
{
    ...
    pDstImage->ppu8Plane[0] =
        (unsigned char *)malloc(pDstImage->aHeight * pDstImage->aWidth * 3 / 2);
    if (pDstImage->ppu8Plane[0]) {
        pDstImage->ppu8Plane[1] = pDstImage->ppu8Plane[0] + pDstImage->aHeight * pDstImage->aWidth;
...
void MiaNodeIntSenseCapBokeh::prepareRelightImage(MIIMAGEBUFFER &dstimage, WAOFFSCREEN &srtImage)
{
    dstimage.i32Width = srtImage.i32Width;
    dstimage.i32Height = srtImage.i32Height;
    dstimage.i32Scanline = srtImage.aHeight;
