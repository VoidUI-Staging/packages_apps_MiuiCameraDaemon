...
S32 NodeMetaData::GetAEInfo(camera_metadata_t *mdata, ARC_HDR_AEINFO *pAeInfo)
{
    DEV_IF_LOGE_RETURN_RET(pAeInfo == NULL, RET_ERR_ARG, "arg err");
    void *pData = NULL;
    S32 ret = GetMetaData(mdata, "com.qti.sensorbps.gain", 0, &pData, NULL);
    ...
    pAeInfo->fADRCGainMin = MiHDRCommonAEParams[0].adrc_min;
    pAeInfo->fADRCGainMax = MiHDRCommonAEParams[0].adrc_max;
    //    DEV_LOGI("fLuxIndex: %.2f fISPGain=%.2f fADRCGain=%.2f ", pAeInfo->fLuxIndex,
    //    pAeInfo->fISPGain, pAeInfo->fADRCGain);
    return RET_OK;
}
...
S32 NodeMetaData::GetFaceRect(camera_metadata_t *mdata, DEV_IMAGE_BUF *inputMain, U32 rotateAngle,
                              ARC_BHDR_FACEINFO &face_info)
{
    if (mdata == NULL || inputMain == NULL || face_info.lFaceOrient == NULL ||
    ...
    ret = GetCropRegion(mdata, &cropRegion);
    DEV_IF_LOGE_RETURN_RET(ret != RET_OK, ret, "GetMetadata err");

    void *pData = NULL;
    ret = GetMetaData(mdata, (char *)"xiaomi.snapshot.faceRect", 0, &pData, NULL);
    DEV_IF_LOGW_RETURN_RET((ret != RET_OK || pData == NULL), RET_OK, "No face");

    FaceResult *pFaceResult = reinterpret_cast<FaceResult *>(pData);

    face_info.nFace = pFaceResult->faceNumber;
    for (int i = 0; i < pFaceResult->faceNumber && i < HDR_RAW_MAX_FACE_NUMBER; ++i) {
        UINT32 dataIndex = 0;
        ...
        face_info.rcFace[i].left = tmpDstFaceRect.left;
        face_info.rcFace[i].top = tmpDstFaceRect.top;
        face_info.rcFace[i].bottom = tmpDstFaceRect.top + tmpDstFaceRect.height;
        face_info.rcFace[i].right = tmpDstFaceRect.left + tmpDstFaceRect.width;
        switch (rotateAngle) {
        case 0:
            face_info.lFaceOrient[i] = 1;
            break;
        case 90:
            face_info.lFaceOrient[i] = 2;
            break;
        case 180:
            face_info.lFaceOrient[i] = 3;
            break;
        case 270:
            face_info.lFaceOrient[i] = 4;
            break;
        default:
            face_info.lFaceOrient[i] = 0;
            break;
        }
    }
...
S32 NodeMetaData::GetfocusROI(camera_metadata_t *mdata, DEV_IMAGE_BUF *inputMain,
                              DEV_IMAGE_RECT *focusROI)
{
    DEV_IF_LOGE_RETURN_RET((mdata == NULL || inputMain == NULL || focusROI == NULL), RET_ERR_ARG,
                           "arg err");
    S32 ret = RET_OK;
    ...
    void *pData = NULL;
    ret = GetMetaData(mdata, NULL, ANDROID_CONTROL_AF_REGIONS, &pData, NULL);
...
S32 NodeMetaData::GetfocusTP(camera_metadata_t *mdata, DEV_IMAGE_BUF *inputMain,
                             DEV_IMAGE_POINT *focusTP)
{
    DEV_IF_LOGE_RETURN_RET((mdata == NULL || inputMain == NULL || focusTP == NULL), RET_ERR_ARG,
                           "arg err");
    S32 ret = RET_OK;
    DEV_IMAGE_RECT focusROI = {0};
    ret = GetfocusROI(mdata, inputMain, &focusROI);
