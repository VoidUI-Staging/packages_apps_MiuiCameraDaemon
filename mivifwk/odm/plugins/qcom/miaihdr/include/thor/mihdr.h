...
enum MIAI_HDR_ALGO_MODE
{
    MIAI_UNDEFINED_HDR = 0,
    MIAI_RAW_HDR = 1,
    MIAI_YUV_HDR = 2
};
...
union HDRMetaData {
...
#if defined(ZEUS_CAM) || defined(CUPID_CAM) || defined(THOR_CAM) || defined(ZIZHAN_CAM) || \
    defined(INGRES_CAM) || defined(DITING_CAM)
        uint32_t cropSize[4];
        char ev0_preprocess;
#endif
    } config;
    ...
    bool is_selfie() const {
        return config.camera_id < 10;
    }

    std::string cameraId() const {
        switch(config.camera_id) {
        case SELFIE_CAMERA1: return "SELFIE_CAMERA1";
        case SELFIE_CAMERA2: return "SELFIE_CAMERA2";
        case REAR_CAMERA1:  return "REAR_WIDE";
        case REAR_TELE: return "REAR_TELE";
        case REAR_WIDE_2: return "REAR_WIDE_2";
        case REAR_EXTRA_WIDE: return "REAR_EXTRA_WIDE";
        case REAR_MACRO: return "REAR_MACRO";
        case REAR_WIDE_108M: return "REAR_WIDE_108M";
        default: return "UNKNOWN_CAMERA_ID";
        }
    }
};
...
{
    MInt8  eyenum;
    FDPoint left[MAX_EYELAND_POINT];
    FDPoint right[MAX_EYELAND_POINT];
};
...
int nightProcess(const void* inputs, int input_size, void* output, int rotate, int64_t timestamp, const void* pAeInfo, const HDRMetaData &meta, callback_fun fun, void *pCallBackObj, callback_ion ion_release);

int nightProcess_eyeinfo(const void *inputs, int input_size, void *output, int rotate, int64_t timestamp, const void *pAeInfo, const HDRMetaData &meta, LPARC_LLHDR_FACEINFO face_info, const eye_info eyes[],callback_fun fun, void *pCallBackObj, callback_ion ion_release);

int nightPreviewProcess(const void* input, void* output, int rotate, int64_t timestamp, const void* pAeInfo, void* pCheckInfo);

void dumpYuvData(const char* suffix, const unsigned char* img, int width, int height, int uv_offset, int pitch);

void dumpYuvToJpg(const char* suffix, const unsigned char* img, int width, int height, int uv_offset, int pitch);

void dumpYuvData(const char* suffix, const unsigned char* img, int width, int height, int uv_offset, int pitch, const char* directory);

void dumpYuvData(const char* suffix, const unsigned char* img, const unsigned char* img_uv, int width, int height, int pitch, const char* directory);

void dumpYuvToJpg(const char* suffix, const unsigned char* img, int width, int height, int uv_offset, int pitch, const char* directory);

void drawTextInYuv(const char* text, unsigned char* img, int width, int height, int uv_offset, int pitch, int fontscale=4, int thickness=8, int orient = 90);

void drawOrientTextInYuv(unsigned char* img, int width, int height, int uv_offset, int pitch, int fontscale=4, int thickness=8, int orient = 90, std::string cameraId = "");
...
void processRawData(mihdr::MiImageBuffer *buffer, int ev, int frameNum, GainInfo* info, HDRMetaData *hdrMetaData);
