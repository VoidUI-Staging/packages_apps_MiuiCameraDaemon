...
typedef struct _tag_HDR_EVLIST_AEC_INFO {
    LPARC_HDR_AEINFO pAEInfo;
    MInt32 ev_list[HDR_EV_MAX_LEN];
    MFloat total_gain[HDR_EV_MAX_LEN];
    MFloat sensitivity[HDR_EV_MAX_LEN];
    MUInt64 expossure_time[HDR_EV_MAX_LEN];
} HDR_EVLIST_AEC_INFO, *LPHDR_EVLIST_AEC_INFO;

typedef struct _tag_ARC_HDR_Userdata {
    MRECT                    rtCropRect;           // [out] The effective rectangle of hdr result
    MInt32                  i32DoCropResize;   // [in]  Do crop-reisze or not, 0: do not crop-resize result image, 1: do crop-resize result image
...
************************************************************************/
typedef MRESULT (*ARC_HDR_FNPROGRESS) (
...
************************************************************************/
ARC_HDR_API MVoid  ARC_HDR_SetCallback(
    MHandle                hHandle,            // [in]  The algorithm engine
