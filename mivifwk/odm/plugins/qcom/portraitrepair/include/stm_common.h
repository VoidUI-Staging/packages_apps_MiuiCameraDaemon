...
enum {
    ...
    STM_CONFIG_QUALITY_STRENGTH, ///< 人像过滤强度,参数为整数类型, 目的是过滤掉质量太差的人脸，
                                 ///< 避免效果不协调， 级别越高表示过滤越严格；
                                 ///< 只对修复人脸个数有影响， 对效果没有影响， 可选值参考{@see
                                 ///< stm_quality_strength_t}，默认值STM_QUALITY_LOW
    STM_CONFIG_MAX_FACES, ///< 参与人像修复的最大人脸个数,参数为整数类型,可设置范围[1-10],
                          ///< 默认为10，超出范围将被截取
    STM_CONFIG_QUALITY_DEBUG, ///< 质量模块调试开关,参数为整数类型，默认0关闭，1
                              ///< 开启，开启后清晰度高也强制都进入修复
    ...
    STM_CONFIG_BLEND_WEIGHT_FACE, ///< 面部非五官区域融合比重, 参数为浮点类型，取值范围[0 -
                                  ///< 1]，默认值1.0
    STM_CONFIG_BLEND_WEIGHT_BORDER ///< 边缘特征融合比重, 参数为浮点类型，取值范围[0 -
                                   ///< 1]，默认值0.65
};
