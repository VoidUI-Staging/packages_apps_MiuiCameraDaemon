...
#define Model_PATH \
    "/odm/etc/camera/facesr_hd_composite_ref_4.3.15_qcom8550_qnn_segqnn_qnn2.3_qualityv35r2.model"
#endif
#define Rear_Classic_JSON_PATH  "/odm/etc/camera/portraitrepaire_rear_class.json"
#define Rear_Vivid_JSON_PATH    "/odm/etc/camera/portraitrepaire_rear_live.json"
#define Front_Classic_JSON_PATH "/odm/etc/camera/portraitrepaire_front_class.json"
#define Front_Vivid_JSON_PATH   "/odm/etc/camera/portraitrepaire_front_live.json"

#define IMG_DUMP_PATH "/data/vendor/camera/PortraitRepairDump/"

static mutex g_algomutex;
static mutex g_destroymutex;
static condition_variable g_condProcessWait;
static condition_variable g_condDestroyWait;
...
bool PortraitRepair::isEnabled(MiaParams settings)
{
    ...
    const char *aiportraitdeblur = "xiaomi.protraitrepair.enabled";
    VendorMetadataParser::getVTagValue(settings.metadata, aiportraitdeblur, &data);
    if (NULL != data) {
        mPortraitEnable = *static_cast<bool *>(data);
        MLOGI(Mia2LogGroupPlugin, "PortraitRepair getMetaData mPortraitEnable  %d",
              mPortraitEnable);
    } else {
        MLOGW(Mia2LogGroupPlugin, "PortraitRepair can not found tag xiaomi.protraitrepair.enabled");
    }

    data = NULL;
    bool hasFace = false;
    const char *faceRectangles = "android.statistics.faceRectangles";
    VendorMetadataParser::getVTagValue(settings.metadata, faceRectangles, &data);
    if (NULL != data) {
        hasFace = true;
    } else {
    ...
    if (mEnableStatus && mPortraitEnable && hasFace &&
        (mFrameworkCameraId != CAM_COMBMODE_REAR_MACRO)) {
        isEnable = true;
    ...
    MLOGI(Mia2LogGroupPlugin,
          "PortraitRepair mPortraitEnable %d enable: %d, g_algostatus %d handle %p, CameraId: %d",
          mPortraitEnable, isEnable, g_algostatus, g_algohandle, mFrameworkCameraId);
    return isEnable;
}

int PortraitRepair::preProcess(PreProcessInfo preProcessInfo)
{
...
ProcessRetStatus PortraitRepair::processRequest(ProcessRequestInfo *pProcessRequestInfo)
{
    ...
    data = NULL;
    int32_t thermalLevel = 0; // 0: <35; 1: 35~37; 2: 41~43; 3: 43~45; 4: 46~47; 5: 48~50
    VendorMetadataParser::getVTagValue(metadata, "xiaomi.thermal.thermalLevel", &data);

    if (NULL != data) {
        thermalLevel = *reinterpret_cast<int32_t *>(data);
        MLOGI(Mia2LogGroupPlugin, "PortraitRepair get thermalLevel %d", thermalLevel);
    }
    ...
    if (mNodeInterface.pSetResultMetadata != NULL) {
        ...
        exifmsg << "PortraitRepair:" << mPortraitEnable << " thermalLevel:" << thermalLevel
                << " algostatus:" << g_algostatus << " isoverlap:" << isProcessface
                << " runningTaskNum:" << pProcessRequestInfo->runningTaskNum
                << " {result:" << processParam.PortraitRepairResult
                << " faces:" << processParam.faceexifinfo.faces << " afRext:" << afRect.left << "_"
                << afRect.top << "_" << afRect.right << "_" << afRect.bottom << " iso:" << iso
                << " rotate:" << rotate << " luxIndex:" << luxIndex << " scene:" << sceneDetected
                << " time:" << end - startTime;
        for (int i = 0; i < processParam.faceexifinfo.faces; i++) {
            exifmsg << " {blur:" << processParam.faceexifinfo.blur[i]
                    ...
                    << " rects:" << processParam.faceexifinfo.rects[i][0] << ","
                    << processParam.faceexifinfo.rects[i][1] << ","
                    << processParam.faceexifinfo.rects[i][2] << ","
    ...
    int set_tag_result = VendorMetadataParser::updateVTagValue(
        metadata, "xiaomi.protraitrepair.enabled", &mPortraitEnable, 1, &outmetadata);
...
ProcessRetStatus PortraitRepair::processRequest(ProcessRequestInfoV2 *pProcessRequestInfo)
{
    ...
    float ispGain = 1.0;
    data = NULL;
    VendorMetadataParser::getVTagValue(metadata, "com.qti.sensorbps.gain", &data);
    if (NULL != data) {
        ispGain = *static_cast<float *>(data);
        MLOGD(Mia2LogGroupPlugin, "PortraitRepair get ispGain %f", ispGain);
    }
    ...
    int set_tag_result = VendorMetadataParser::updateVTagValue(
        metadata, "xiaomi.protraitrepair.enabled", &mPortraitEnable, 1, NULL);
    MLOGD(Mia2LogGroupPlugin, "mPortraitEnable %d set_tag_result %d", mPortraitEnable,
          set_tag_result);
...
int PortraitRepair::CopyYuvBuffer(yuv_image_t *hOutput, yuv_image_t *hInput)
{
    ...
    MLOGI(Mia2LogGroupCore, "CopyImage input %d, %d, %d, output %d, %d, %d, format 0x%x 0x%x",
    ...
    if ((hOutput->format == YUV_IMAGE_FMT_NV21) || (hOutput->format == YUV_IMAGE_FMT_NV12)) {
        ...
        MLOGW(Mia2LogGroupCore, "%s:%d unknow format: %d", __func__, __LINE__, hOutput->format);
        ret = -1;
    }
    return ret;
}
                                 ...
                                 int *faceNum, ChiRectEXT *afRect)
{
    ...
    if (physicalMeta != NULL) {
        VendorMetadataParser::getVTagValue(physicalMeta, cropInfo, &data);
    } else {
    ...
    ChiRectEXT AFRect;
    if (physicalMeta != NULL) {
        VendorMetadataParser::getTagValue(physicalMeta, ANDROID_CONTROL_AF_REGIONS, &data);
    } else {
    ...
    const char *faceRect = "xiaomi.snapshot.faceRect";
    if (physicalMeta != NULL) {
        VendorMetadataParser::getVTagValue(physicalMeta, faceRect, &data);
    } else {
        MLOGW(Mia2LogGroupPlugin, "PortraitRepair physicalMeta  is null use logicalMeta.");
    ...
    if (NULL != data) {
        ...
        *faceNum = tmpData->faceNumber;
        ...
        MLOGI(Mia2LogGroupPlugin, "PortraitRepairt get face failed or not detect face.");
    }
}
} // namespace mialgo2
