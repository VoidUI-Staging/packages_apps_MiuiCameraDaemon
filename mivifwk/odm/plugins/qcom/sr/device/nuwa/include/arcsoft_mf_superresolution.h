...
/**
 * @brief This function is implemented by the caller, registered with any time-consuming processing
 * functions, and will be called periodically during processing so the caller application can obtain
 * the operation status (i.e., to draw a progress bar), as well as determine whether the operation
 * should be canceled or not
 */
typedef MRESULT (*ARC_MFSR_FNPROGRESS)(MLong lProgress, MLong lStatus, MVoid *pParam);
 ...
 */
#define ARC_MFSR_CAMERA_TYPE_REAR_WIDE_0    0x00 // IMX989_CLASSIC
#define ARC_MFSR_CAMERA_TYPE_REAR_WIDE_0_RM 0x01 // IMX989_RM_CLASSIC
#define ARC_MFSR_CAMERA_TYPE_REAR_WIDE_1    0x10 // IMX989
#define ARC_MFSR_CAMERA_TYPE_REAR_WIDE_1_RM 0x11 // IMX989_RM
#define ARC_MFSR_CAMERA_TYPE_REAR_TELE_0    0x20 // JN1_CLASSIC
#define ARC_MFSR_CAMERA_TYPE_REAR_TELE_0_RM 0x21 // JN1_RM_CLASSIC
#define ARC_MFSR_CAMERA_TYPE_REAR_TELE_1    0x30 // JN1
#define ARC_MFSR_CAMERA_TYPE_REAR_TELE_1_RM 0x31 // JN1_RM
...
MFSUPERRESOLUTION_API MVoid ARC_MFSR_SetLogLevel(
    ...
    MInt32 i32LogLevel, FN_ARC_MFSR_android_log_print fn_log_print = MNull);
...
#define ARC_MFSR_VendorTag_dump_path "persist.vendor.camera.arcsoft.mfsr.dump.path"
/**
 ...
 */
MFSUPERRESOLUTION_API MRESULT ARC_MFSR_Init(MHandle *phEnhancer, LPASVLOFFSCREEN pSrcImg,
 ...
 */
MFSUPERRESOLUTION_API MRESULT ARC_MFSR_UnInit(MHandle *phEnhancer);
 ...
 * @return MOK if success, otherwise fail
 */
MFSUPERRESOLUTION_API MRESULT ARC_MFSR_SetCropRegion(MHandle hEnhancer, MRECT *pRtScaleRegion);
 ...
 * @return MOK if success, otherwise fail
 */
MFSUPERRESOLUTION_API MRESULT ARC_MFSR_PreProcess(MHandle hEnhancer, LPASVLOFFSCREEN pSrcImg,
                                                  MInt32 i32ImgIndex,
 ...
 * @return MOK if success, otherwise fail
 */
MFSUPERRESOLUTION_API MRESULT ARC_MFSR_GetDefaultParam(MHandle hEnhancer, LPARC_MFSR_PARAM pParam);

/**
 * @brief This function is used to set reference index, if *pRefIndex < 0, will used default
 * reference index inner
 * @param hEnhancer [in] The algorithm engine
 * @param pRefIndex [in/out] The reference image index, if *pRefIndex < 0, this parameter will be
 * set as real index by algorithm
 * @return MOK if success, otherwise fail
 */
MFSUPERRESOLUTION_API MRESULT ARC_MFSR_SetRefIndex(MHandle hEnhancer, MInt32 *pRefIndex);

/**
 * This function is used to set face info outside
 * @param hEnhancer [in] The algorithm engine
 * @param pFaceInfo [in] The face info of reference image, if pFaceInfo is nullptr, will use face
 * info detected bt sdk
 * @return MOK if success, otherwise fail
 */
MFSUPERRESOLUTION_API MRESULT ARC_MFSR_SetFaceInfo(MHandle hEnhancer,
 ...
 * @return MOK if success, otherwise fail
 */
MFSUPERRESOLUTION_API MRESULT ARC_MFSR_Process(MHandle hEnhancer, LPARC_MFSR_PARAM pSRParam,
                                               LPASVLOFFSCREEN pDstImg);
 ...
 */
MFSUPERRESOLUTION_API MVoid ARC_MFSR_SetCallback(MHandle hEnhancer, ARC_MFSR_FNPROGRESS fnCallback,
                                                 MVoid *pUserData);
