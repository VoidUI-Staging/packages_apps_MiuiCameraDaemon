...
S32 NodeMetaData::GetRotateAngle(camera_metadata_t *mdata, U32 *RotateAngle)
{
    ...
    DEV_LOGI("rotateAngle = %d", *RotateAngle);
    return RET_OK;
}
...
S32 NodeMetaData::GetExtremeDarkSeResult(camera_metadata_t *mdata, S32 *extremeDarkSeResult)
{
    DEV_IF_LOGE_RETURN_RET(extremeDarkSeResult == NULL, RET_ERR_ARG, "arg err");
    S32 ret = RET_OK;
    void *pData = NULL;
    ret = GetMetaData(mdata, "xiaomi.ai.asd.extremeDarkSeResult", 0, &pData, NULL);
    DEV_IF_LOGE_RETURN_RET(ret != RET_OK, ret, "GetMetadata err");
    DEV_IF_LOGE_RETURN_RET(pData == NULL, RET_ERR, "get metadata err");
    (*extremeDarkSeResult) = (S32)(*reinterpret_cast<int *>(pData));
    return ret;
}
                                 ...
                                 S32 DstReferInputHeight, DEV_IMAGE_RECT *DstRect)
{
    DEV_IF_LOGW_RETURN_RET(
        ...
        RET_ERR, "srcRect[%d %d] srcRefer[%d %d] inputmain[%d %d]", SrcRect.width, SrcRect.height,
        SrcRefer.width, SrcRefer.height, DstReferInputWidth, DstReferInputHeight);
...
S32 NodeMetaData::GetArcsoftFaceInfo(camera_metadata_t *pMdata, DEV_IMAGE_BUF *inputMain,
                                     ARC_DCR_FACE_PARAM &faceRect)
{
#define MAX_FACE_NUM_BOKEH 10
    DEV_LOGI(
        "m_algoDepthSetParams.faceInfo.faceParam.prtFaces %p "
        "m_algoDepthSetParams.faceInfo.faceParam.pi32FaceAngles %p",
        faceRect.prtFaces, faceRect.pi32FaceAngles);
    if (pMdata == NULL || inputMain == NULL || faceRect.pi32FaceAngles == NULL ||
        faceRect.prtFaces == NULL) {
        faceRect.i32FacesNum = 0;
        DEV_LOGE("arg err");
        return RET_ERR_ARG;
    }
    S32 ret = RET_OK;
    ...
    void *pData = NULL;
    ret = GetMetaData(pMdata, "xiaomi.snapshot.faceRect", 0, &pData, NULL);
    DEV_IF_LOGW_RETURN_RET((ret != RET_OK || pData == NULL), RET_OK, "No face");

    FaceResult *pFaceResult = static_cast<FaceResult *>(pData);
    faceRect.i32FacesNum = pFaceResult->faceNumber;
    if (faceRect.i32FacesNum == 0) {
        return ret;
    }
    S32 orient = 0;
    switch (rotateAngle) {
    case 0:
        orient = ARC_DCR_FOC_0;
        break;
    case 90:
        orient = ARC_DCR_FOC_90;
        break;
    case 180:
        orient = ARC_DCR_FOC_180;
        break;
    case 270:
        orient = ARC_DCR_FOC_270;
        break;
    default:
        orient = ARC_DCR_FOC_0;
        break;
    ...
    for (int i = 0; i < pFaceResult->faceNumber && i < MAX_FACE_NUM_BOKEH; ++i) {
        ...
        ret = MapRectToInput(tmpFaceRect, cropRegion, inputMain->width, inputMain->height,
                             &tmpDstFaceRect);
        DEV_LOGI("MapRectToInput is %d", ret);
        faceRect.prtFaces[i].left = tmpDstFaceRect.left;
        faceRect.prtFaces[i].top = tmpDstFaceRect.top;
        faceRect.prtFaces[i].right = tmpDstFaceRect.left + tmpDstFaceRect.width - 1;
        faceRect.prtFaces[i].bottom = tmpDstFaceRect.top + tmpDstFaceRect.height - 1;
        faceRect.pi32FaceAngles[i] = orient;
...
#define MAX_FACE_NUM_BOKEH 10
    ...
    //    DEV_LOGI("MaxFaceRect [%d %d %d %d][%d]",
    //    faceRect->x,faceRect->y,faceRect->width,faceRect->height,faceRect->angle);
    return ret;
}
                             ...
                             DEV_IMAGE_POINT *focusTP)
{
    DEV_IF_LOGE_RETURN_RET((pMdata == NULL || inputMain == NULL || focusTP == NULL), RET_ERR_ARG,
                           "pMdata %p, inputmain %p focusTp %p", pMdata, inputMain, focusTP);
    S32 ret = RET_OK;
