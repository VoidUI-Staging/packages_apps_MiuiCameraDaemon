#ifndef _MIAIRAWHDR_H_
#define _MIAIRAWHDR_H_
...
#define FRAME_MAX_INPUT_NUMBER 6

enum { MODE_HDR = 0, MODE_LLHDR, MODE_SUPER_LOW_LIGHT_HDR, MODE_SRHDR, MODE_RAWHDR };
...
private:
    void process_rawhdr_xmi(struct MiImageBuffer *input, struct MiImageBuffer *output,
    ...
    void setRawOffScreen(HDR_RAW_OFFSCREEN *pImg, struct MiImageBuffer *miBuf,
                         MIAI_HDR_RAWINFO &rawInfo);
    uint32_t getRawType(uint32_t rawFormat, ColorFilterPattern rawPattern);
