...
DrawPoint(ASVLOFFSCREEN *pOffscreen, MPOINT xPoint, MInt32 nPointWidth, MInt32 iType)
{
    if (MNull == pOffscreen) {
        return;
    }
    ...
    if (pOffscreen->u32PixelArrayFormat == ASVL_PAF_NV21 ||
        ASVL_PAF_NV12 == pOffscreen->u32PixelArrayFormat) {
        MUInt8 *pByteYTop = pOffscreen->ppu8Plane[0] + nStartY * pOffscreen->pi32Pitch[0] + nStartX;
        MUInt8 *pByteVTop =
            pOffscreen->ppu8Plane[1] + (nStartY / 2) * (pOffscreen->pi32Pitch[1]) + nStartX;
...
static void __attribute__((unused))
DrawRect(LPASVLOFFSCREEN pOffscreen, MRECT RectDraw, MInt32 nLineWidth, MInt32 iType)
{
    if (MNull == pOffscreen) {
        return;
    }
    ...
    if (pOffscreen->u32PixelArrayFormat == ASVL_PAF_NV21 ||
        ASVL_PAF_NV12 == pOffscreen->u32PixelArrayFormat) {
        // draw the top line
        MUInt8 *pByteYTop = pOffscreen->ppu8Plane[0] +
                            (nTop - nPoints / 2) * pOffscreen->pi32Pitch[0] + nLeft - nPoints / 2;
        MUInt8 *pByteVTop = pOffscreen->ppu8Plane[1] +
                            ((nTop - nPoints / 2) / 2) * pOffscreen->pi32Pitch[1] + nLeft -
                            nPoints / 2;
        MUInt8 *pByteYBottom = pOffscreen->ppu8Plane[0] +
                               (nTop + nRecrH - nPoints / 2) * pOffscreen->pi32Pitch[0] + nLeft -
                               nPoints / 2;
        MUInt8 *pByteVBottom = pOffscreen->ppu8Plane[1] +
                               ((nTop + nRecrH - nPoints / 2) / 2) * pOffscreen->pi32Pitch[1] +
                               nLeft - nPoints / 2;
        for (int i = 0; i < nPoints; i++) {
            pByteVTop = pOffscreen->ppu8Plane[1] +
                        ((nTop + i - nPoints / 2) / 2) * pOffscreen->pi32Pitch[1] + nLeft -
                        nPoints / 2;
            pByteVBottom = pOffscreen->ppu8Plane[1] +
                           ((nTop + i + nRecrH - nPoints / 2) / 2) * pOffscreen->pi32Pitch[1] +
                           nLeft - nPoints / 2;
        ...
        for (int i = 0; i < nRecrH; i++) {
            MUInt8 *pByteYLeft = pOffscreen->ppu8Plane[0] + (nTop + i) * pOffscreen->pi32Pitch[0] +
                                 nLeft - nPoints / 2;
            MUInt8 *pByteYRight = pOffscreen->ppu8Plane[0] + (nTop + i) * pOffscreen->pi32Pitch[0] +
                                  nLeft + nRectW - nPoints / 2;
            MUInt8 *pByteVLeft = pOffscreen->ppu8Plane[1] +
                                 (((nTop + i) / 2) * pOffscreen->pi32Pitch[1]) + nLeft -
                                 nPoints / 2;
            MUInt8 *pByteVRight = pOffscreen->ppu8Plane[1] +
                                  (((nTop + i) / 2) * pOffscreen->pi32Pitch[1]) + nLeft + nRectW -
                                  nPoints / 2;
...
static void __attribute__((unused))
DrawRectROI(LPASVLOFFSCREEN pOffscreen, CHIRECT rt, MInt32 nLineWidth, MInt32 iType)
{
    int l = rt.left;
    int t = rt.top;
