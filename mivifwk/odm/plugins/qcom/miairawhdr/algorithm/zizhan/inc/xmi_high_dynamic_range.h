...
typedef struct __tag_HDR_RAW_OFFSCREEN
{
    ...
    MInt32 pi32Pitch[4];
    MInt32 fd[4];
} HDR_RAW_OFFSCREEN, *LPHDR_RAW_OFFSCREEN;
typedef struct _tag_HDR_RAW_INPUTINFO
{
    MInt32 i32ImgNum;
    HDR_RAW_OFFSCREEN InputImages[HDR_RAW_MAX_LEN];
} HDR_RAW_INPUTINFO, *LPHDR_RAW_INPUTINFO;
 ...
 * for front camera
 ************************************************************************/
XMI_HDR_API MRESULT XMI_HDR_RAW_Init( // return MOK if success, otherwise fail
    MHandle *phHandle                 // [out] The algorithm engine will be initialized by this API
);

XMI_HDR_API MRESULT XMI_HDR_RAW_Uninit( // return MOK if success, otherwise fail
    MHandle *phHandle // [in/out] The algorithm engine will be un-initialized by this API
);

XMI_HDR_API MRESULT XMI_HDR_RAW_PreProcess( // return MOK if success, otherwise fail
    MHandle hHandle                         // [in]  The algorithm engine
);

XMI_HDR_API MRESULT XMI_HDR_RAW_Process( // return MOK if success, otherwise fail
    MHandle hHandle,                     // [in]  The algorithm engine
    LPHDR_RAW_INPUTINFO pInputImages,    // [in]  The input images array
    MIAI_HDR_RAWINFO *pRawInfo,          // [in]  The AE info of normal input image
    LPHDR_RAW_OFFSCREEN pDstImg,         // [out]  The offscreen of result image
...
XMI_HDR_API MRESULT XMI_HDR_PreProcess( // return MOK if success, otherwise fail
    MHandle hHandle,                    // [in]  The algorithm engine
    LPXMI_HDR_INPUTINFO pInputImages,   // [in]  The input images array
    LPXMI_HDR_AEINFO pAEInfo,           // [in]  The AE info of normal input image
...
/************************************************************************
The function is used to detect how many frames to shot, and Ev for each frame, just for raw hdr.

pi32EVInfo and pi32EVArray are output params, for example:
...
pi32EVInfo(frame num, algo type):  { [3, 1],      [6, 0],                 [3, 2]      }
pi32EVArray:                       { [-18, 0, 6], [-36, -18, 0, 0, 0, 0], [-18, 0, 6] }

************************************************************************/

XMI_HDR_API MRESULT XMI_Multi_HDR_Mode_ParameterDetect(
    LPASVLOFFSCREEN pPreviewImg, // [in] The input Preview data same ev as normal
    LPXMI_LLHDR_AEINFO pAEInfo,  // [in] The AE info of input image
    MInt32 i32Mode, // [in] The model name for camera, 0 for rear camera, 0x100 for front camera
    MInt32 i32Type, // [in] The HDR type, XMI_TYPE_QUALITY_FIRST(0), XMI_TYPE_PERFORMANCE_FIRST(1)
    ...
    MInt32 *i32MotionState, // [in] The motion detection result
    HDRMetaData *hdrMetaData, // [in/out] The meta data info of normal input image
    MInt32 **pi32EVInfo,      // [out] The number of shot image, it must be 1, 2 or 3.
    MInt32 **pi32EVArray,     // [out] The ev value for each image
    MInt32 *pi32HdrDetect     // [out] checker detect result
);
