...
const char *ecoCloudInfoPath = "/data/vendor/camera/EcoCloudInfo.json";
...
Return<int32_t> BGService::setEventCallback(int32_t clientId, const sp<IEventCallback> &callback)
{
    ...
    if (callback != nullptr) {
        ...
        if (!linked.isOk()) {
            MLOGE(Mia2LogGroupService,
                  "Transaction error in linking to mEventCallback death: %s, clientId(%d)",
                  linked.description().c_str(), clientId);
        } else if (!linked) {
            MLOGE(Mia2LogGroupService, "Unable to link to mEventCallback death notifications");
        }
    } else {
        MLOGE(Mia2LogGroupService, "App HIDL EventCallback function is empty, callback=%p",
              callback.get());
    }
    MLOGI(Mia2LogGroupService, "clientId: %d, callback: %p", clientId, callback.get());
...
{
    std::string msg = info;
    std::ofstream configFile(ecoCloudInfoPath, std::fstream::out);
    if (!configFile.is_open()) {
        MLOGE(Mia2LogGroupService, "open failed! file: %s", ecoCloudInfoPath);
        return Void();
    }

    configFile.write(msg.c_str(), msg.size());
    configFile.close();
    MLOGI(Mia2LogGroupService, "write cloud info success! file: %s", ecoCloudInfoPath);
    return Void();
}
...
void BGService::serviceDied(uint64_t cookie, const wp<hidl::base::V1_0::IBase> &who __unused)
{
    ...
    MLOGW(Mia2LogGroupService, "BGService IEventCallback client %d serviceDied!!", clientId);
                                                ...
                                                std::string &version, int32_t clientId)
{
    mVersion = version;
    if (callback == nullptr) {
        MLOGE(Mia2LogGroupService, "App HIDL EventCallback function is empty with clientId=%d",
              clientId);
        return int32_t{};
    }
    {
...
bool BGServiceWrap::onResultCallback(const ResultData &result)
{
    ...
    if (callback == nullptr) {
        MLOGW(Mia2LogGroupService, "[Mock]App HIDL ver %s EventCallback function is empty!",
              mVersion.c_str());
        return false;
    }

    Return<bool> ret = false;
    MLOGI(Mia2LogGroupService,
          "[Mock]Callback via %s interface, fwkframe(%d), timestamp(%" PRId64 ") isParallel: %d",
    ...
    if (!ret.isOk()) {
        MLOGW(Mia2LogGroupService, "[Mock]Transaction error in IEventCallback::onCompleted: %s",
...
bool BGServiceWrap::notifySnapshotAvailability(int32_t available)
{
    ...
    if (callback == nullptr) {
        MLOGE(Mia2LogGroupService, "App HIDL ver %s EventCallback function is empty!",
              mVersion.c_str());
        return false;
    }

    Return<bool> ret = false;
    MLOGI(Mia2LogGroupService, "snapshot availability=%d", available);

    ret = callback->notifySnapshotAvailability(available);
    if (!ret.isOk()) {
        MLOGE(Mia2LogGroupService, "Transaction error in IEventCallback::onCompleted: %s",
              ret.description().c_str());
        return false;
    }
...
{
    MLOGI(Mia2LogGroupService, "fileName = %s, fwkFrame:%u buffer come back", fileName.c_str(),
          frameNumber);
...
{
    MLOGI(Mia2LogGroupService, "fileName = %s, fwkFrame:%u snapshot fail", fileName.c_str(),
          frameNumber);
