...
public:
    ...
    virtual bool setFromMetadata(const Metadata &metadata, const Metadata &staticInfo);
...
bool ExifUtilsImpl::initialize(uint32_t width, uint32_t height)
{
    reset();
    ...
    property_get("ro.product.manufacturer", prop, "");
    manufacturer_.assign(prop, strlen(prop));

    memset(prop, 0, sizeof(prop));
    property_get("ro.product.model", prop, "");
    model_.assign(prop, strlen(prop));
    // XIAOMI ADD END

    exif_data_ = exif_data_new();
    if (exif_data_ == nullptr) {
        MLOGE(LogGroupHAL, "allocate memory for exif_data_ failed");
        return false;
    }
...
bool ExifUtilsImpl::setAperture(float aperture)
{
    ...
    SET_RATIONAL(EXIF_IFD_EXIF, EXIF_TAG_APERTURE_VALUE,
                 static_cast<uint32_t>(round(apexValue * kRationalPrecision)), kRationalPrecision);
    return true;
}
...
bool ExifUtilsImpl::setDateTime(const struct tm &t)
{
    ...
    if (result != sizeof(str) - 1) {
        MLOGW(LogGroupHAL, "Input time is invalid");
        return false;
    }
                                        ...
                                        uint32_t sensor_width, uint32_t sensor_height)
{
    ...
    if (zoomRatio <= noZoomThreshold) {
        ...
        SET_RATIONAL(EXIF_IFD_EXIF, EXIF_TAG_DIGITAL_ZOOM_RATIO,
                     static_cast<uint32_t>(round(zoomRatio * kRationalPrecision)),
                     kRationalPrecision);
    }
    return true;
}
...
bool ExifUtilsImpl::setExposureTime(float exposure_time)
{
    SET_RATIONAL(EXIF_IFD_EXIF, EXIF_TAG_EXPOSURE_TIME,
                 static_cast<uint32_t>(round(exposure_time * kRationalPrecision)),
                 kRationalPrecision);
    return true;
}
...
bool ExifUtilsImpl::setFNumber(float f_number)
{
    SET_RATIONAL(EXIF_IFD_EXIF, EXIF_TAG_FNUMBER,
                 static_cast<uint32_t>(round(f_number * kRationalPrecision)), kRationalPrecision);
    return true;
}

bool ExifUtilsImpl::setFocalLength(float focal_length)
{
    uint32_t numerator = static_cast<uint32_t>(round(focal_length * kRationalPrecision));
                                             ...
                                             float sensor_size_y)
{
    ...
    float sensorDiagonal = sqrt(sensor_size_x * sensor_size_x + sensor_size_y * sensor_size_y);
    sensorDiagonal = std::max(sensorDiagonal, minSensorDiagonal);
    float focalLength35mmFilm = round(focal_length * filmDiagonal / sensorDiagonal);
...
bool ExifUtilsImpl::setGpsAltitude(double altitude)
{
    ...
    if (!refEntry) {
        MLOGE(LogGroupHAL, "Adding GPSAltitudeRef exif entry failed");
        return false;
    }
    if (altitude >= 0) {
        *refEntry->data = 0;
    ...
    std::unique_ptr<ExifEntry> entry =
        addVariableLengthEntry(EXIF_IFD_GPS, tag, EXIF_FORMAT_RATIONAL, 1, sizeof(ExifRational));
    if (!entry) {
        exif_content_remove_entry(exif_data_->ifd[EXIF_IFD_GPS], refEntry.get());
        MLOGE(LogGroupHAL, "Adding GPSAltitude exif entry failed");
        return false;
    }
...
bool ExifUtilsImpl::setGpsLatitude(double latitude)
{
    ...
    if (!refEntry) {
        MLOGE(LogGroupHAL, "Adding GPSLatitudeRef exif entry failed");
        return false;
    }
    if (latitude >= 0) {
    ...
    if (!entry) {
        exif_content_remove_entry(exif_data_->ifd[EXIF_IFD_GPS], refEntry.get());
        MLOGE(LogGroupHAL, "Adding GPSLatitude exif entry failed");
        return false;
    }
...
bool ExifUtilsImpl::setGpsLongitude(double longitude)
{
    ...
    std::unique_ptr<ExifEntry> refEntry =
        addVariableLengthEntry(EXIF_IFD_GPS, refTag, EXIF_FORMAT_ASCII, 2, 2);
    if (!refEntry) {
        MLOGE(LogGroupHAL, "Adding GPSLongitudeRef exif entry failed");
        return false;
    }
    if (longitude >= 0) {
    ...
    if (!entry) {
        exif_content_remove_entry(exif_data_->ifd[EXIF_IFD_GPS], refEntry.get());
        MLOGE(LogGroupHAL, "Adding GPSLongitude exif entry failed");
        return false;
    }
...
bool ExifUtilsImpl::setGpsTimestamp(const struct tm &t)
{
    ...
    if (!entry) {
        MLOGE(LogGroupHAL, "Adding GPSDateStamp exif entry failed");
        return false;
    }
    ...
    if (!entry) {
        MLOGE(LogGroupHAL, "Adding GPSTimeStamp exif entry failed");
        return false;
    }
...
bool ExifUtilsImpl::setMaxAperture(float aperture)
{
    ...
    SET_RATIONAL(EXIF_IFD_EXIF, EXIF_TAG_MAX_APERTURE_VALUE,
                 static_cast<uint32_t>(round(maxAperture * kRationalPrecision)),
                 kRationalPrecision);
    return true;
}
...
bool ExifUtilsImpl::setSubjectDistance(float diopters)
{
    ...
    if (diopters > kInfinityDiopters) {
        ...
        numerator = static_cast<uint32_t>(round(focusDistance * kRationalPrecision));
...
bool ExifUtilsImpl::generateApp1()
{
    ...
    if (!app1_length_) {
        MLOGE(LogGroupHAL, "Allocate memory for app1_buffer_ failed");
        return false;
    }
    /*
    ...
    if (app1_length_ > 65533) {
        MLOGE(LogGroupHAL, "The size of APP1 segment is too large len:%u", app1_length_);
        destroyApp1();
        return false;
    }
    return true;
                                                                 ...
                                                                 unsigned int size)
{
    ...
    if (!mem) {
        MLOGE(LogGroupHAL, "Allocate memory for exif entry failed");
        return nullptr;
    }
...
bool ExifUtilsImpl::setShort(ExifIfd ifd, ExifTag tag, uint16_t value, const std::string &msg)
{
    ...
    if (!entry) {
        MLOGE(LogGroupHAL, "Adding '%s' entry failed", msg.c_str());
        return false;
    }
...
bool ExifUtilsImpl::setFromMetadata(const Metadata &metadata, const Metadata &staticInfo)
{
    struct timespec tp;
    struct tm time_info;
    ...
    if (!setDateTime(time_info)) {
        MLOGE(LogGroupHAL, "setting data time failed.");
        return false;
    }
    ...
    if (entry.count) {
        ...
        if (!setFocalLength(focal_length)) {
            MLOGE(LogGroupHAL, "setting focal length failed.");
            return false;
        }
        ...
        if (sensorSizeEntry.count == 2) {
            if (!setFocalLengthIn35mmFilm(focal_length, sensorSizeEntry.data.f[0],
                                          sensorSizeEntry.data.f[1])) {
                MLOGE(LogGroupHAL, "setting focal length in 35mm failed.");
                return false;
            }
        }
    } else {
        MLOGD(LogGroupHAL, "Cannot find focal length in metadata.");
    }
    ...
    if (metadata.exists(ANDROID_SCALER_CROP_REGION) &&
        ...
        if (!setDigitalZoomRatio(entry.data.i32[2], entry.data.i32[3], activeArrayEntry.data.i32[2],
                                 activeArrayEntry.data.i32[3])) {
            MLOGE(LogGroupHAL, "setting digital zoom ratio failed.");
            return false;
        }
    }
    ...
    if (metadata.exists(ANDROID_JPEG_GPS_COORDINATES)) {
        ...
        if (entry.count < 3) {
            MLOGE(LogGroupHAL, "Gps coordinates in metadata is not complete.");
            return false;
        }
        if (!setGpsLatitude(entry.data.d[0])) {
            MLOGE(LogGroupHAL, "setting gps latitude failed.");
            return false;
        }
        if (!setGpsLongitude(entry.data.d[1])) {
            MLOGE(LogGroupHAL, "setting gps longitude failed.");
            return false;
        }
        if (!setGpsAltitude(entry.data.d[2])) {
            MLOGE(LogGroupHAL, "setting gps altitude failed.");
            return false;
        }
    }
    ...
    if (metadata.exists(ANDROID_JPEG_GPS_PROCESSING_METHOD)) {
        ...
        if (!setGpsProcessingMethod(method_str)) {
            MLOGE(LogGroupHAL, "setting gps processing method failed.");
            return false;
        }
    }
    ...
    if (time_available && metadata.exists(ANDROID_JPEG_GPS_TIMESTAMP)) {
        ...
        if (gmtime_r(&timestamp, &time_info)) {
            if (!setGpsTimestamp(time_info)) {
                MLOGE(LogGroupHAL, "setting gps timestamp failed.");
                return false;
            }
        } else {
            MLOGE(LogGroupHAL, "Time tranformation failed.");
            return false;
        }
    }
    ...
    if (staticInfo.exists(ANDROID_CONTROL_AE_COMPENSATION_STEP) &&
        ...
        camera_metadata_ro_entry stepEntry = staticInfo.find(ANDROID_CONTROL_AE_COMPENSATION_STEP);
        if (!setExposureBias(entry.data.i32[0], stepEntry.data.r[0].numerator,
                             stepEntry.data.r[0].denominator)) {
            MLOGE(LogGroupHAL, "setting exposure bias failed.");
            return false;
        }
    }
    ...
    if (metadata.exists(ANDROID_SENSOR_EXPOSURE_TIME)) {
        ...
        if (!setExposureTime(exposure_time)) {
            MLOGE(LogGroupHAL, "setting exposure time failed.");
            return false;
        }

        if (!setShutterSpeed(exposure_time)) {
            MLOGE(LogGroupHAL, "setting shutter speed failed.");
            return false;
        }
    }
    ...
    if (metadata.exists(ANDROID_LENS_FOCUS_DISTANCE)) {
        ...
        if (!setSubjectDistance(entry.data.f[0])) {
            MLOGE(LogGroupHAL, "setting subject distance failed.");
            return false;
        }
    }
    ...
    if (metadata.exists(ANDROID_SENSOR_SENSITIVITY)) {
        ...
        if (!setIsoSpeedRating(static_cast<uint16_t>(iso))) {
            MLOGE(LogGroupHAL, "setting iso rating failed.");
            return false;
        }
    }
    ...
    if (metadata.exists(ANDROID_LENS_APERTURE)) {
        ...
        if (!setFNumber(entry.data.f[0])) {
            MLOGE(LogGroupHAL, "setting F number failed.");
            return false;
        }
        if (!setAperture(entry.data.f[0])) {
            MLOGE(LogGroupHAL, "setting aperture failed.");
            return false;
        }
    }
    ...
    if (!setColorSpace(kSRGBColorSpace)) {
        MLOGE(LogGroupHAL, "setting color space failed.");
        return false;
    }
    ...
    if (staticInfo.exists(ANDROID_LENS_INFO_AVAILABLE_APERTURES)) {
        ...
        if (!setMaxAperture(entry.data.f[0])) {
            MLOGE(LogGroupHAL, "setting max aperture failed.");
            return false;
        }
    }
    ...
    if (staticInfo.exists(ANDROID_FLASH_INFO_AVAILABLE)) {
        ...
        uint8_t flashState = flashStateEntry.count > 0 ? flashStateEntry.data.u8[0]
                                                       : ANDROID_FLASH_STATE_UNAVAILABLE;
        uint8_t aeMode =
            aeModeEntry.count > 0 ? aeModeEntry.data.u8[0] : ANDROID_CONTROL_AE_MODE_OFF;

        if (!setFlash(entry.data.u8[0], flashState, aeMode)) {
            MLOGE(LogGroupHAL, "setting flash failed.");
            return false;
        }
    }
    ...
    if (metadata.exists(ANDROID_CONTROL_AWB_MODE)) {
        ...
        if (!setWhiteBalance(entry.data.u8[0])) {
            MLOGE(LogGroupHAL, "setting white balance failed.");
            return false;
        }
    }
    ...
    if (metadata.exists(ANDROID_CONTROL_AE_MODE)) {
        ...
        if (!setExposureMode(entry.data.u8[0])) {
            MLOGE(LogGroupHAL, "setting exposure mode failed.");
            return false;
        }
    }
    if (time_available) {
        ...
        if (snprintf(str, sizeof(str), "%03ld", tp.tv_nsec / 1000000) < 0) {
            MLOGE(LogGroupHAL, "Subsec is invalid: %ld", tp.tv_nsec);
            return false;
        }
        if (!setSubsecTime(std::string(str))) {
            MLOGE(LogGroupHAL, "setting subsec time failed.");
            return false;
        }
    }
