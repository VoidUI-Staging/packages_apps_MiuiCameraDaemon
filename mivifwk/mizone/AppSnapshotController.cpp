    ...
    : mAppStatus{ACTIVATED}, mThreadExit{false}, mTimer{Timer::createTimer()}
{
    ...
    memset(&mHdrStatus, 0, sizeof(StatusData));
    memset(&mSEStatus, 0, sizeof(StatusData));
}
            ...
            if (updatedAppStatus != mAppStatus) {
                ...
                if (updatedAppStatus == MUTED) {
                    ...
                    mTimer->runAfter(mWaitTime, [this]() {
                        MLOGE(LogGroupHAL,
                              "[AppSnapshotController]: app has been muted for too long time(10s), "
                              "force to awaken it!!");
...
void AppSnapshotController::UpdateHdrStatus(bool status, int frameNum)
{
    mHdrStatus.isEnable = status;
    mHdrStatus.frameNum = frameNum;
}

void AppSnapshotController::UpdateSEStatus(bool status, int frameNum)
{
    mSEStatus.isEnable = status;
    mSEStatus.frameNum = frameNum;
}

void AppSnapshotController::GetSEStatus(bool &status, int &frameNum)
{
    status = mSEStatus.isEnable;
    frameNum = mSEStatus.frameNum;
}

void AppSnapshotController::GetHdrStatus(bool &status, int &frameNum)
{
    status = mHdrStatus.isEnable;
    frameNum = mHdrStatus.frameNum;
}
