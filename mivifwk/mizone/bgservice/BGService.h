...
class CloudServiceWrap
{
public:
    static CloudServiceWrap *getInstance();

    std::string getCloudInfo();

    void setCloudInfo(std::string info);

private:
    CloudServiceWrap() = default;
    ~CloudServiceWrap() = default;
    std::string mCloudInfo;
    std::mutex mCloudInfoMutex;
};
